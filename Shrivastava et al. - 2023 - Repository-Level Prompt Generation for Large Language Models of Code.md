...
Disha Shrivastava1 2 Hugo Larochelle1 2 3 4 Daniel Tarlow1 5 3  
\`\`\`  
\#\# Abstract
...
code, data, and trained checkpoints athttps:  
//github.com/shrivastavadisha/  
repo\_level\_prompt\_generation.  
\`\`\`  
\#\# 1\. Introduction
...
\`\`\`  
(^1) Mila (^2) Université de Montréal (^3) Google (^4) CIFAR Associate  
Fellow^5 McGill University. Correspondence to: Disha Shrivastava  
\<dishu.905@gmail.com\>.  
Proceedings of the 40 thInternational Conference on Machine  
Learning, Honolulu, Hawaii, USA. PMLR 202, 2023\. Copyright  
2023 by the author(s).  
As opposed to thepretrain-finetuneparadigm,prompting  
...
et al., 2021), has been deployed as part of GitHub Copilot^1 ,  
...
In this work, we address this problem by proposingRepo-  
...
repository. In RLPG, the choice ofwherefrom andwhat  
to takefrom the repository is specified by a set ofprompt  
proposals. For example, one of the prompt proposals can be  
(^1) https://copilot.github.com/

\# arXiv:2206.12839v3 \[cs.LG\] 5 Jun 2023
...
repository as input, the prompt proposal classifier predicts a prompt proposal. The context from the predicted prompt proposalp= 14,  
...
this by coming up with a neural network calledPrompt  
Proposal Classifier (PPC)thatlearnsto select a prompt pro-  
...
are not publicly available (e.g. OpenAI provides accessonly  
to the generated output from Codex via an APIhttps:

\`\`\`  
//openai.com/blog/openai-codex/and no ac-  
cess to model weights and data is provided), making these  
techniques less useful under this scenario. RLPG addresses  
this limitation by generating prompts assuming onlyblack-  
box accessto the LLM. Even for cases where we have access  
...
out portion (ortarget hole) starting from the position of  
...
file is likely to be invoked). ThePrompt Composeruses the  
context from the predicted prompt proposal and combines it  
with thedefault Codex context, i.e., code prior to the position  
of the hole in the current file. The resulting prompt con-  
sists of the method nameInitializeToAssignment  
...
\- We propose a framework called theRepo-Level Prompt  
    Generator (RLPG)thatlearnsto generate prompts con-  
...
from the prompt proposal). A prompt proposal is specified

\`\`\`  
by a prompt source and a prompt context type. We mention  
each of these along with their motivation below.  
Prompt Source:For a target hole position, a prompt source  
determines fromwhereshould we take code that will be part  
...
mineswhatcode to take from the prompt source. We pro-  
pose seven different prompt context types (Appendix B.  
has examples of each type):

\`\`\`  
1.Post Lines (PL):Take all the lines after the target hole  
line till the end of the current file^2.  
...
we get a total of 63 prompt proposals (see Appendix B.  
...
ing prompt sourceImportand prompt context typeMNB.  
...
sifier is to predict the prompt proposalpthat will lead to  
success, where success happens when the predicted holeˆh  
exactly matches the target holeh. This task is formulated  
...
Training: For each target holeh, we generate a ground-  
truth vectorYh= \[yph\]Mp=1which is a multi-hot vector of  
sizeM, whereMis the total number of prompt proposals.  
This vector is obtained by feeding the prompt generated  
from prompt proposalpinto Codex and then seeing whether  
hˆ=h. If there is a match, we say that the prompt proposalp  
is successful. For holeh, if a prompt proposalpis applicable

(^2) We also conducted experiments (Appendix D.3) where we  
take lines starting from the 4th line after the hole.  
and leads to success,yhp= 1and will be zero otherwise.  
For each holeh, we obtain a maskThwhereTph= 1when  
pis applicable or zero otherwise. The overall training loss  
Lcan be expressed as the sum of individual hole lossesLh:
...
ber of applicable prompt proposals forh,Nis the total  
number of holes encountered while training andBCEcor-  
...
used to obtain the predictionyˆph.  
\`\`\`  
\`\`\`  
RLPG-H:LetHhbe thehole windowthat includes code  
present around the holehexcluding the hole itself. In our  
work, we take two lines before the hole position, the code up  
to the hole position and two lines after the hole position. We  
use a pretrained modelFφto obtain a context representation  
vector of sizeZ, whereZis the dimension of the hidden  
...
the parametersφare frozen during training. The RLPG-H  
model takes the context representation of the hole window  
and projects it to the prompt proposal space of sizeM  
...
\`\`\`  
ˆyhp=P(yhp= 1|Hh)  
\= sigmoid(W^2 (relu(W^1 (Fφ(Hh)) \+b^1 )) \+b^2 )(1)  
\`\`\`  
\`\`\`  
...
Given a particular holeh, letCphdenote the prompt pro-  
posal context from prompt proposalp. Intuitively, if the  
hole window contains variables (e.g. identifiers) that are  
similar to the variables in the prompt proposal context, then  
there are chances thathmight occur somewhere inCph. The  
similarity is modeled using a multiheaded attention mecha-  
nism (Vaswani et al., 2017), by treating the projected hole  
window representation as a queryQhand the projected  
prompt proposal context representationKphas a key. The  
valueVphis the same as the key.  
\`\`\`  
\`\`\`  
Qh=Fφ(Hh), Kph=Fφ(Cph), Vph=Fφ(Chp)  
\`\`\`  
\`\`\`  
The output from the multi-headed attention module,  
MultiHead(Qh,Kph,Vph)is fed to moduleGconsisting  
...
et al., 2021). Instead, we scraped Google Codehttps:  
...
different folder. We mark all such copies as duplicates
...
Java^3 that enables us to get the AST of a file and query it.  
...
directory structure inside the repository (see Appendix 3\.  
for more details).  
\`\`\`  
\`\`\`  
...
model. In particular, we used thecode-davinci-  
...
\`\`\`  
(^3) https://github.com/tree-sitter/tree-sitter-java
...
entire prompt.  
2.Oracle:Using the ground-truth vectorYh(mentioned  
in Section 2.2). The prompt generated corresponds  
to using any of the successful prompt proposals (i.e.,  
yph= 1). Since this information is not available at  
...
query and prompt proposal contexts being the search  
\`\`\`  
\`\`\`  
...
8.Random NN:Same asRandom, except that amongst  
...
will call thissuccess rate(SR) going forward.  
\`\`\`  
\`\`\`  
3.3. Results  
\`\`\`  
...
Random 58.13 \-1.  
Random NN 58.98 0\.  
File-level BM25 63.14 7\.  
Identifier Usage (Random) 64.93 10\.  
Identifier Usage (NN) 64.91 10\.  
Fixed Prompt Proposal 65.78 12\.  
RLPG-BM25 66.41 13\.  
RLPG-H 68.51 16\.  
RLPG-R 67.80 15\.  
\`\`\`  
...
prompt proposal or RLPG. We see that File-level BM  
...
have a human-in-the-loop who has been givenkattempts to  
prompt the LLM and then choose one of thekpredictions.  
We wanted to see how the performance of our framework  
varies with \#attempts under this setting. This corresponds to  
usingkprompts generated with top-kprompt proposals (one  
prompt per proposal) and marking success if any of thek  
prompts lead to success. The top part of Figure 2 shows the  
variation of SR over the validation data with the value ofk.  
...
the figure, we notice that as we increase the value ofk, the  
...
the character-level edit distance^4 as a metric. Table 4 reports  
...
(^4) https://pypi.org/project/editdistance/  
26.62% relative improvement.  
...
on thecode-cushman-001model from OpenAI^5. This  
model supports a context length of up to 2048 tokens, which  
is half the context length ofcode-davinci-001and  
...
as prompts forcode-cushman-001in order to get the  
...
els trained on labels fromcode-cushman-001would  
...
different code completion models (code-cushman-  
...
\`\`\`  
(^5) https://platform.openai.com/docs/models/codex
...
prompt from multiple prompt proposals (see Appendix D.  
...
\`\`\`  
(^6) https://openai.com/product/gpt-

\#\# References

Allamanis, M. The adverse effects of code duplication in ma-  
chine learning models of code. InProceedings of the 2019  
...
open-source autoregressive language model. InProceed-  
...
ing evaluation for few-shot NLP. InAdvances in Neural  
...
with generative language models. InProceedings of the  
...
code. InProceedings of the 37th International Confer-  
...
optimization. InInternational Conference on Learning  
...
for parameter-efficient prompt tuning. InProceedings of  
...
ous prompts for generation. InProceedings of the 59th  
...
code. InDeep Learning for Code Workshop, 2022a.

Pashakhanloo, P., Naik, A., Wang, Y., Dai, H., Maniatis, P.,  
and Naik, M. Codetrek: Flexible modeling of code using  
an extensible relational representation. InInternational  
...
LMs with mixtures of soft prompts. InProceedings of  
...
ence. InProceedings of the 16th Conference of the Eu-  
...
with automatically generated prompts. InEmpirical Meth-  
ods in Natural Language Processing (EMNLP), 2020\.  
Shrivastava, D., Larochelle, H., and Tarlow, D. On-the-  
fly adaptation of source code models. InNeurIPS 2020  
...
design. In27th International Conference on Intelligent  
...
of prompt tuning in code intelligence. InProceedings of  
...
els for code understanding and generation. InProceedings  
...
orizing transformers. InInternational Conference on  
...
To collect the hole completion data, we scraped Google Code^7 for repositories tagged with the language “Java”. Then we  
...
was done using the suggested tokenizer from OpenAI^8.

A.2. Creation of Data for Repo-Level Prompt Proposals

We used the tree-sitter API for Java^9 to get the parse-tree of an individual file in a repo. To get information at a repo-level,  
...
(^7) https://code.google.com/archive/  
(^8) https://huggingface.co/docs/transformers/model\_doc/gpt2\#transformers.GPT2TokenizerFast  
(^9) https://github.com/tree-sitter/tree-sitter-java
...
ter the linemg.InitializeToAssignment(CurrentAssignments())till we reach the end of the file  
...
InitializeToAssignment(line 1),a(line 1),currentAssignment\_(line 2),a( line 2),clone(line  
2), alreadyInitialized\_(line 3),justOneRound\_(line 4).  
3.Type Identifiers (TI): Type Identifiers define the type of an identifier. For example, in the code snippetclass  
DPAffinityPropagation extends AffinityPropagation , \[ AffinityPropagation is la-  
beled as a type identifier. Similarly in the snippetDPAPParameters parameters\_;, DPAPParametersis a  
type identifier.  
4.Field Declarations (FD): The variables of a class type are introduced by field declarations. For example,  
double\[\]\[\] mHijMujT\_; and MessageValuePair\[\]\[\] sortedMHijMujTs\_; are examples of  
field declarations.  
...
\- Prompt Proposal Context Formatting:We can format the prompt proposal context before giving it to the Prompt  
    Composer. We experimented with the following options:  
       1\. class\_name: append \[class name of the file\] at the beginning of the prompt proposal context taken from each file that  
          is part of the prompt source. For example, if we are taking prompt proposal context from two import filesf 1 andf 2 ,  
          the prompt proposal context will be formatted as: \[class name off 1 \] prompt proposal context fromf 1 \+ space \+  
          \[class name off 2 \] prompt proposal context fromf 2\. We use this when the prompt proposal context types are MN, I,  
          TI, FD and SL.

2\. class\_method\_name: we apply this only when the prompt proposal context type is MNB. We append method names  
...
et al., 2020\) as our pretrained modelFφto obtain the representation of hole window. The size of the representation  
(corresponding to the hidden dimension of the\[CLS\]token) is 768.W^1 ∈R^512 ×^768 ,b^1 \= 512,W^2 ∈R^63 ×^512 ,b^2 \= 63\.

\#\#\#\# C.2. RLPG-R

We used Adam (Kingma & Ba, 2015\) optimizer with a learning rate of 3e-4 and batch size of 64\. We used CodeBERT (Feng  
et al., 2020\) as our pretrained modelFφto obtain the representation of hole window and prompt proposal context. The size of  
...
\`\`\`  
MultiHead(Qh,Khp,Vph) \=WOconcat(head 1 ,...headτ)  
where headi=Att(WiQQh,WiKKph,WiVVph)  
\`\`\`  
In the above equations,dkis the dimension of the key,WiQ,WiK,WiVare the query, key and value projection matrices,τis  
the number of heads andWOis the linear projection that combines the heads. The projection matricesWiQ∈Rdq×dmodel,  
WiK∈Rdk×dmodel,WiV∈Rdv×dmodel,WO∈Rdmodel×τdv. For the multihead attention, we useddk=dq=dv= 32,  
τ \= 4anddmodel= 768,Wr∈R^63 ×^768 andbp= 63\. For each head, we perform a scaled dot-product attention. G  
...
baselines use the Okapi BM25 implementation with default parameters given by the pip package rank-bm25 0.2.2^10.  
For file-level BM25, if the file context exceeds the allocated context length, we truncate from the back.

(^10) https://pypi.org/project/rank-bm25/
...
Oracle 79.63 35.58 80.24 32\.  
Random 58.13 \-1.02 58.95 \-2.  
Random NN 58.98 0.43 60.04 \-0.  
File-level BM25 63.14 7.51 64.28 6\.  
Identifier Usage (Random) 64.93 10.55 67.83 11\.  
Identifier Usage (NN) 64.91 10.52 67.94 12\.  
Fixed Prompt Proposal 65.78 12.00 68.01 12\.  
RLPG-BM25 66.41 13.07 68.15 12\.  
RLPG-H 68.51 16.65 69.26 14\.  
RLPG-R 67.80 15.44 69.28 14\.  
\`\`\`  
...
(repo-wise)  
Codex (Chen et al., 2021\) 58.73 \- 60.64 \-  
Post Lines (immediate line after the hole) 65.78 12.00 68.01 12\.  
Post Lines (skipping three lines after the hole) 65.11 10.86 66.42 9\.  
\`\`\`  
D.4. Composition of prompt proposals

Table 10 shows the performance of the two versions of RLPG when we compose the prompt proposal context fromlprompt  
...
correspond to taking context from a single prompt proposal (i.e., the training setting). The drop in success rate withl= 2  
...
(repo-wise)  
Codex (Chen et al., 2021\) 58.73 \- 60.64 \-  
RLPG-H (l= 1\) 68.51 16.65 69.26 14\.  
RLPG-R (l= 1\) 67.80 15.44 69.28 14\.  
RLPG-H (l= 2\) 67.07 14.20 67.87 11\.  
RLPG-R (l= 2\) 66.57 13.35 67.88 11\.  
RLPG-H (l= 5\) 66.60 13.40 67.91 11\.  
RLPG-R (l= 5\) 65.78 12.01 67.69 11\.  
RLPG-H (l= 10\) 65.53 11.58 67.24 10\.  
RLPG-R (l= 10\) 63.59 8.27 65.98 8\.  
\`\`\`  
...
Prompt Proposal  
Context  
Either Both Success Rate  
\`\`\`  
\`\`\`  
RLPG-H 26.95 30.95 39.22 18.68 68\.  
RLPG-R 26.82 31.31 38.88 19.24 67\.  
\`\`\`  
...
\#\#\# largemail 1653 75.38 55.11 62.73 63.94 63\.

\#\#\# ftpserverremoteadmin 7323 86.44 66.11 76.09 76.21 76\.

\#\#\# myt5lib 838 91.65 53.58 61.34 73.51 74\.

\#\#\# seamlets 4890 92.74 62.25 62.72 71.55 74\.

\#\#\# gloodb 10000 91.07 57.50 57.50 70.32 72\.

\#\#\# jjskit 9043 80.36 65.61 72.18 72.00 72\.

\#\#\# mobileexpensetracker 2298 75.94 57.88 67.28 66.84 66\.

\#\#\# gfsfa 10000 80.55 57.33 57.33 59.28 65\.

\#\#\# swe574-group3 2029 76.79 54.46 66.19 65.16 64\.

\#\#\# strudem-sicsa 6131 77.83 64.96 72.55 73.25 73\.

\#\#\# soap-dtc 1370 81.24 64.82 70.73 71.61 72\.

\#\#\# openprocesslogger 7191 81.06 62.19 71.77 72.22 72\.

\#\#\# tapestry-sesame 397 72.54 45.84 61.21 60.71 63\.

\#\#\# exogdx 735 84.76 63.81 75.51 75.92 76\.

\#\#\# designpatternjavapedro 1069 78.30 54.82 64.36 63.99 68\.

\#\#\# quidsee 3020 81.66 60.79 69.50 70.36 70\.

\#\#\# healpix-rangeset 4734 63.54 48.71 54.67 54.94 55\.

\#\#\# sol-agent-platform 10000 73.76 58.22 65.72 65.65 65\.

\#\#\# rsbotownversion 10000 75.23 57.89 65.58 66.22 66\.
...
affinity\_propagation\_java 1466 79.54 59.14 70.33 70.26 70.26  
...
file (i.e. MaximizingGibbsSampler.java). Note thatmg. before the hole position indicates that a method used  
...
\- RLPG \= an hasConverged\_;vs Codex \= an converged\_;  
\- RLPG \= \_\[i\]\[j\] \= \-Double.MAX\_VALUE; vs Codex \= \_\[i\]\[j\] \= 0;
