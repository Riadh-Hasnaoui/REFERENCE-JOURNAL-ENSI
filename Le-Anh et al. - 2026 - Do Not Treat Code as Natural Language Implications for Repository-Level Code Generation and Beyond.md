...
with complex large repository context, show that Hydra achieves state-of-the-art performance across open-  
...
\`\`\`  
\# arXiv:2602.11671v1 \[cs.SE\] 12 Feb 2026
...
(February 2026), 24 pages. https://doi.org/10.1145/
...
To bridge these gaps, we propose Hydra, a repository-level code generation framework that  
explicitly exploits the structure and dependencies in a codebase. Specifically, Hydra is designed  
...
\#\#\# ○ 2 We introduce a retrieval mechanism centered on call-graph dependencies, namely dependency-
...
\`\`\`  
\#\#\# ○^3 We propose Hydra Retriever, ahybrid retrieval strategy that combinesDARwith similarity-
...
that Hydra generalizes across model families and sizes. It achieves state-of-the-art results,  
...
Given a prompt𝑃and a knowledge corpus𝐾representing the target repository, the final generated  
code,𝐶final, is produced by the function:  
𝐶final=G(𝑃 ⊕R(𝑃,𝐾)) (1)  
\`\`\`  
...
information is integrated with the original prompt𝑃. The generator functionG(·)then synthesizes  
the final code based on this augmented input.  
In most prior work, the retrieval componentR(·)has been approached through either sparse  
...
query 𝑞 and the code snippet 𝑑.
...
where:  
\`\`\`  
\- 𝑘 1 is a saturation parameter controlling the impact of term frequency (typically 1\. 2 ≤ 𝑘 1 ≤  
    2\. 0 ), 𝑏 is the length-normalization parameter ( 0 ≤ 𝑏 ≤ 1 ),  
\- 𝑎𝑣𝑔𝑑𝑙 is the average document length across the collection,  
(2)Semantic dense retrieval using embedding similarity is typically implemented with a  
pretrained encoder (e.g., UniXCoder \[ 15 \] or CodeBERT \[ 12 \]), which maps both the query𝑞  
and candidate document𝑑into vector representations. A similarity score (commonly cosine  
...
gap between a query and its most useful context.A^3 \-CodeGen \[ 33 \] extracts, fuses, and feeds  
...
using Python’sast^1 module. This allows us to represent the codebase as a graph of nodes, where  
...
triplets(𝑞,𝑑+,𝑑−), where𝑞denotes the query,𝑑+is a set of true dependencies, and𝑑−is the set  
of irrelevant candidates. To construct such data, we mined Python repositories from GitHub and  
processed them using the structure-aware indexing procedure described in Section 3.2. The full data  
preparation pipeline is detailed in Section 3.3.1. With this dataset, we fine-tune an encoder, framing  
dependency detection as a binary classification task over query–candidate pairs. Specifically, the  
...
\`\`\`  
Require: Filtered set of Python repositories 𝑅𝑓 𝑖𝑙𝑡𝑒𝑟𝑒𝑑.  
Ensure: Training dataset 𝐷 with(𝑞,𝑑+,𝑑−) triplets.  
1: 𝐷 ←∅  
2: for all repository 𝑟 ∈ 𝑅𝑓 𝑖𝑙𝑡𝑒𝑟𝑒𝑑do  
3: F,C,V ← ExtractAllCodeUnits(𝑟)⊲ Function, Class, and Variable Pools  
4: for all function 𝑓𝑖∈ F do  
5: 𝑓𝑖𝑙𝑒𝑠𝑟𝑒𝑙← GetFileOf(𝑓𝑖)∪ GetImportedFilesBy(𝑓𝑖)  
6: 𝑆𝑐𝑜𝑛𝑡𝑒𝑥𝑡← FilterUnitsByFiles(F,C,V, 𝑓𝑖𝑙𝑒𝑠𝑟𝑒𝑙)  
7: 𝑞𝑖← GetSignature(𝑓𝑖)+ GetDocstring(𝑓𝑖)⊲ Create the query  
8: 𝑑𝑖+← AnalyzeDependenciesAST(𝑓𝑖,𝑆𝑐𝑜𝑛𝑡𝑒𝑥𝑡)⊲ Positive set  
9: 𝑑𝑖−← 𝑆𝑐𝑜𝑛𝑡𝑒𝑥𝑡\\(𝑑𝑖+∪{𝑓𝑖})⊲ Negative set  
10: Append(𝑞𝑖,𝑑𝑖+,𝑑𝑖−) to 𝐷  
11: end for  
12: end for  
13: return 𝐷  
\`\`\`  
...
The detailed process is presented in Algorithm 1\. To create the query𝑞, we extract function nodes set  
F𝑡𝑎𝑟𝑔𝑒𝑡from the crawled repositories and use their function signature together with the docstring  
...
components (functions, classes, and variables). For each anchor function𝑓𝑖∈ F𝑡𝑎𝑟𝑔𝑒𝑡, components  
that are explicitly invoked by𝑓𝑖are added to the positive set𝑑+𝑖, while all others serve as negative  
samples 𝑑𝑖−, to form(𝑞𝑖,𝑑𝑖+,𝑑𝑖−).  
3.3.2 Retriever Fine-Tuning Strategy. Once the training dataset is constructed (Section 3.3.1), we  
fine-tune an encoder (UniXCoder \[ 15 \]) to detect the true dependencies𝑑+for a given query𝑞. We  
...
independent input and train the encoder to classify concatenated input𝑝⊕𝑑into {0,1}, where  
1 denotes a valid dependency and 0 otherwise.  
...
detection. As a result, this process yielded a dataset of approximately 564 , 740 data samples. The  
...
\`\`\`  
\`\`\`  
def snake\_case\_to\_camel(...)  
...  
\`\`\`  
\`\`\`  
def camel\_case\_to\_snake(...):  
...  
...
if not is\_string(input\_string):  
raise InvalidInputError(input\_string)  
if not is\_snake\_case(input\_string, separator):  
return input\_string  
...  
\`\`\`  
\`\`\`  
...
an ambiguous optimization landscape. For instance, functionFunc\_Aand functionFunc\_Bmay  
both be valid dependencies for a query, requiring their embeddings to be pulled closer. However, for  
a different query,Func\_Amight be a true dependency, a positive sample, whileFunc\_Bis a negative  
...
exclusively within the training set to match the number of positive samples, creating a balanced 1:  
ratio. The validation and test sets were kept in their original imbalanced state to ensure a realistic  
evaluation.  
\`\`\`  
\`\`\`  
3.4 Hydra Retriever \- A Hybrid Approach  
\`\`\`  
...
how these components should be used in practice (e.g., how to invoke a function or in which  
...
Indeed, as illustrated in Figure 3, when generating thesnake\_case\_to\_camelfunction, DAR  
successfully identifies the correct dependencies that must be called. At the same time, BM25 can  
find the related functioncamel\_case\_to\_snake, which demonstrates how such dependencies are  
invoked in practice.  
To combine these strengths, we design Hydra Retriever, a hybrid retrieval strategy that  
integrates dependency-aware retrieval (DAR) with similarity-based retrieval (i.e., BM25). By merging  
these complementary contexts, Hydra provides the model with both the necessary building blocks  
...
\`\`\`  
\`\`\`  
Inference process of Hydra: During inference, the target function is passed through both the  
BM25 retriever and the DAR.  
\`\`\`  
\- For BM25, we apply the standard formulation (Equation 2), following common practice by fixing  
    𝑘 1 \= 1\. 5 and 𝑏= 0\. 75\. We then retrieve the top-5 most similar snippets.  
\- For DAR, rather than using only the hard classification labels described in Section 3.3.2, we  
    leverage the prediction probability𝑝of class 1 to retain candidates, thereby reducing the risk of  
    discarding true dependencies. To achieve this, we introduce a threshold hyperparameter𝑇; a  
    candidate dependency is retained if its predicted probability𝑝\> 𝑇. The threshold𝑇is tuned on  
    ...
    (recall 1 ), while keeping the model’s ability to detect false dependencies (recall 0 ) at an acceptable  
       ...
       between recall 1 and recall 0 , scaled by a coefficient𝛼that reflects the class imbalance. The metric  
       is defined as:  
...
dependencies). Due to the class imbalance, the value of 𝛼 is thus determined to be 0.2.  
\`\`\`  
\`\`\`  
Table 1\. BRP scores at different threshold values𝑇 on the validation set.  
\`\`\`  
\`\`\`  
Threshold 0.15 0.20 0.25 0.30 0.35 0.40 0.45 0\.  
BRP 0.8829 0.8852 0.8861 0.8838 0.8800 0.8755 0.8660 0\.  
\`\`\`  
\`\`\`  
As shown in Table 1, a threshold of 0.25 yields the highest BRP score of 0.8861. Therefore, we  
select𝑇= 0\. 25 for our Hydra Retriever during inference.  
\`\`\`  
4 Experiment Setup  
...
is performed as described in Section 3.2. For both indexing strategies, we employ the same BM  
...
retriever (DAR) compared to prior similarity-based methods, including sparse retrieval with BM  
...
the comparison to assess how these three retrieval methodologies, together with Hydra, affect  
performance on the repository-level code generation task.  
RQ3: How effective is Hydra compared to existing state-of-the-art approaches for repository-level code  
generation?  
↩→Setup: We compare Hydra against three state-of-the-art repository-level code generation  
...
RQ4: How does the computational cost of running Hydra compare to state-of-the-art approaches for  
repository-level code generation?  
↩→Setup: We compare the retrieval time of Hydra against the baselines considered in RQ3 to  
...
.format(self.\_\_class\_\_.\_\_name\_\_))  
...
\*\*def\*\* rename\_dont\_move... (self, path, dest):  
def ... \_list\_existing(filesystem, glob, paths):  
if isinstancefor key, value\_list (paths, dictin): paths.items():  
for value in value\_list:  
(^) existing\_pathsif filesystem.isdir.(updatevalue)(:filesystem.listdir(value))  
(^) existing\_pathselif filesystem.isfile.add((valuevalue)):  
....  
\*\*AttributeError: 'LocalFileSystem'  
object has no attribute 'isfile'  
Missing Context  
Qwen2.5-Coder-1.5B-Instruct Solution  
Available Context Fed Into Generator  
Missing Context  
Ground-truth Callee  
Retrieved  
Chunk\*\*  
Fig. 4\. Example of contextual fragmentation under chunk-based retrieval. The retriever provides partial context  
(isdir,listdir) but misses the ground-truth dependencyexists. As a result, the model hallucinates an  
isfile method, leading to an AttributeError.  
...
Model Size Benchmark Indexing Strategy Pass@1 Pass@3 Pass@  
1.5B  
RepoExec  
Chunking-based Indexing 9.13 14.96 17\.  
Structure-Aware Indexing 12.39 16.70 18\.  
DevEval Chunking-based Indexing 4.81 6.60 7\.  
Structure-Aware Indexing 10.32 14.13 15\.  
7B  
RepoExec Chunking-based Indexing 14.03 19.32 21\.  
Structure-Aware Indexing 20.79 27.61 30\.  
DevEval Chunking-based Indexing 8.46 10.88 11\.  
Structure-Aware Indexing 16.67 21.73 23\.  
RQ1: How effective is Structure-Aware Indexing compared to Chunking-Based Indexing?  
...
Figure 4 presents a case study on completing the\_list\_existingfunction, which relies on  
theFileSystemobject. The chunk-based retriever captures fragments containing theisdirand  
listdirmethods but fails to retrieve the criticalexistsmethod. Without this context, the model  
generatesfilesystem.isfile(path), a plausible but hallucinated method inferred by analogy  
toisdir. The resultingAttributeError, illustrated in the traceback, is a direct consequence of  
...
based strategies by a wide margin, with around 2 × gains on DevEval.  
\`\`\`  
\`\`\`  
...
Retriever  
RepoExec DevEval  
Recall Precision F1 Recall Precision F  
Sparse Retriever 0.5378 0.0809 0.1406 0.5734 0.1243 0\.  
Dense Retriever 0.5483 0.0824 0.1432 0.5183 0.1124 0\.  
Dependency-aware Retriever 0.9223 0.1086 0.1957 0.8884 0.1559 0\.  
FRecall CRecall VRecall FRecall CRecall VRecall  
Sparse Retriever 0.6522 0.6667 0.3174 0.7216 0.6562 0\.  
Dense Retriever 0.6957 0.5196 0.3832 0.7557 0.5191 0\.  
Dependency-aware Retriever 0.9420 0.8922 0.9162 0.8718 0.9194 0\.  
\`\`\`  
...
3.3.2) as well as on the RepoExec and DevEval benchmarks. We report Precision, Recall, and F  
...
\`\`\`  
\`\`\`  
Model Size Retriever RepoExec DevEval  
Pass@1 Pass@3 Pass@5 DIR Pass@1 Pass@3 Pass@  
\`\`\`  
\`\`\`  
1.5B  
\`\`\`  
\`\`\`  
Sparse Retriever 12.39 16.70 18.02 39.93 10.32 14.13 15\.  
Dense Retriever 12.73 19.04 21.40 39.86 10.24 13.89 15\.  
Dependency-aware Retriever 13.46 18.02 20.28 43.81 10.43 14.58 16\.  
Hydra Retriever 15.72 21.30 23.38 44.61 10.71 14.50 16\.  
\`\`\`  
\`\`\`  
7B  
\`\`\`  
\`\`\`  
Sparse Retriever 20.79 27.61 30.14 49.67 16.67 21.73 23\.  
Dense Retriever 19.89 26.45 29.58 46.38 15.93 20.92 22\.  
Dependency-aware Retriever 21.97 29.89 32.96 52.24 16.84 21.93 23\.  
Hydra Retriever 23.32 31.32 34.36 53.46 17.27 22.44 24\.  
\`\`\`  
...
to retrieve for similarity-based approaches, with recall below 0\. 4\. To our knowledge, variables  
...
\`\`\`  
\`\`\`  
Fig. 5\. Case study comparing the generated code by Hydra and text-based retrieval ontask\_id18 (RepoExec).  
(a) Target Function Prompt  
\`\`\`  
1 def is\_url(input\_string: Any, allowed\_schemes: Optional\[List\[str\]\] \= None) \-\> bool:  
2 """  
3 Check if a string is a valid URL.  
4 \[...\]  
5 """

\`\`\`  
(b) Solution generated with Hydra  
\`\`\`  
1 def is\_url(input\_string: Any, allowed\_schemes: Optional\[List\[str\]\] \= None) \-\> bool:  
2 \[...\]  
3 if not is\_full\_string(input\_string):  
4 return False  
5 \[...\]

\`\`\`  
(c) Solution generated with BM  
\`\`\`  
1 def is\_url(input\_string: Any, allowed\_schemes: Optional\[List\[str\]\] \= None) \-\> bool:  
2 \[...\]  
3 if not isinstance(input\_string, str):  
4 raise InvalidInputError(input\_string)  
5 \[...\]
...
Hydra Retriever consistently delivers the best overall performance. At the 1.5B scale, Hydra  
...
challenging DevEval benchmark, Hydra achieves Pass@1 scores of 10.71 and 17.27 with the 1.5B  
and 7B models, respectively, outperforming all baselines. These results demonstrate that similarity-  
based context complements dependency context by providing signals on how dependencies are  
...
in Figure 5\. In RepoExec (task\_id18), completingis\_urlrequires validating that the input is a  
proper string via the auxiliary functionis\_full\_string. Hydra, using DAR, successfully retrieves  
...
and often deviate from the intended behavior. This highlights Hydra ’s advantage in retrieving  
...
usage signals, and the hybrid Hydra Retriever leverages both to achieve the strongest results.  
\`\`\`  
RQ3: How effective is Hydra compared to existing state-of-the-art approaches for  
repository-level code generation?

Hydra vs. Baselines. Table 5 compares Hydra with several representative baselines, including  
RepoCoder, RepoFormer, and RLCoder, as well as no-context generation. Across all model families  
and sizes, Hydra consistently delivers the best performance, establishing itself as a new state  
...
Hydra retriever surpasses much larger baselines: it outperforms RepoCoder on RepoExec and  
...
Former, and RLCoder), Hydra significantly reduces critical errors such as NameError, TypeError,  
...
demonstrates the effectiveness of Hydra in preserving code structure, retrieving true dependencies,  
...
that Hydra shifts errors from structural failures toward more nuanced behavioral validation,  
...
\`\`\`  
\`\`\`  
Table 5\. Comparison of Hydra with prior retrieval-based approaches and no-context baselines. Results are  
reported in Pass@1/3/5 using close sourced GPT-4.1 mini and open source Qwen2.5-Coder-1.5B and 7B  
Instruct models.  
\`\`\`  
\`\`\`  
Generator Method RepoExec DevEval  
...
\`\`\`  
\`\`\`  
No Context 21.58 24.42 25.63 19.72 23.19 24\.  
RepoCoder 22.20 26.08 27.89 17.48 23.15 25\.  
RepoFormer 39.15 42.42 43.94 30.89 34.21 35\.  
RLCoder 38.14 42.17 43.38 29.46 32.76 34\.  
Hydra 43.55 45.72 46.48 31.91 35.56 36\.  
\`\`\`  
\`\`\`  
QwenCoder-1.5B-Instruct  
\`\`\`  
\`\`\`  
No Context 5.75 8.31 9.30 3.53 5.20 5\.  
RepoCoder 7.15 11.72 14.37 4.54 8.08 9\.  
RepoFormer 11.15 16.42 18.87 5.58 7.94 8\.  
RLCoder 14.87 21.04 23.94 9.34 12.90 14\.  
Hydra 15.72 21.30 23.38 10.71 14.50 16\.  
\`\`\`  
\`\`\`  
QwenCoder-7B-Instruct  
\`\`\`  
\`\`\`  
No Context 13.30 17.04 18.03 7.10 9.16 10\.  
RepoCoder 14.82 21.99 25.07 6.39 10.63 12\.  
RepoFormer 17.69 25.04 28.45 10.41 13.68 14\.  
RLCoder 20.17 23.69 27.61 13.00 17.67 19\.  
Hydra 23.32 31.32 34.36 17.27 22.44 24\.  
\`\`\`  
\`\`\`  
...
\`\`\`  
(^0) AssertionError NameError TypeError AttributeError SyntaxError ValueError  
100  
200  
300  
400  
...
components are respectively removed from the search space of Hydra. From Table 6, we observe  
...
\`\`\`  
Benchmark Method QwenCoder-1.5B-Instruct QwenCoder-7B-Instruct  
Pass@1 Pass@3 Pass@5 Pass@1 Pass@3 Pass@  
\`\`\`  
\`\`\`  
RepoExec  
\`\`\`  
\`\`\`  
Hydra 15.72 21.30 23.38 23.32 31.32 34\.  
w/o-Function 13.12 15.69 16.62 17.97 24.54 27\.  
w/o-Class 11.94 17.18 19.15 20.73 27.15 30\.  
w/o-Variable 13.01 17.86 20.00 21.01 28.17 30\.  
\`\`\`  
\`\`\`  
DevEval  
\`\`\`  
\`\`\`  
Hydra 10.71 14.50 16.05 17.27 22.44 24\.  
w/o-Function 8.58 12.43 14.30 14.76 19.58 21\.  
w/o-Class 8.24 12.03 14.02 14.15 19.04 21\.  
w/o-Variable 9.21 13.35 15.29 15.72 20.07 21\.  
\`\`\`  
retrieval provide more essential API and structural signals, while variable-level retrieval plays a  
complementary role.

\`\`\`  
\[RQ3\] Summary: Hydra establishes a new state of the art in repository-level code generation,  
...
highlighting the value of high-quality retrieval. Error analysis further shows that Hydra reduces  
...
\`\`\`  
RQ4: How does the computational cost of running Hydra compare to state-of-the-art  
...
By comparison, our proposed Hydra achieves consistently low latency while maintaining stable  
performance across queries, owing to its context-aware retrieval design. Instead of exhaustively  
scanning the entire repository, Hydra anchors retrieval on call-graph dependencies at the function
...
\`\`\`  
\`\`\`  
Method RepoExec DevEval  
Min Max Mean Median Min Max Mean Median  
RLCoder 91.71 2927.08 644.92 347.16 91.49 1824.29 894.40 821\.  
RepoFormer 13.59 27286.85 1766.28 18.44 12.40 34336.94 4432.41 18\.  
RepoCoder 919.64 34072.90 7212.16 5201.46 566.52 24306.62 5096.50 4618\.  
Hydra 0.01 1836.32 362.95 277.80 0.01 21234.01 432.86 221\.  
\`\`\`  
...
code, higher average latency, and frequent extreme outliers. Moreover, Hydra incorporates a  
...
From a complexity perspective, Hydra incurs a retrieval cost of𝑂(𝑐), where𝑐corresponds to the  
size of the current file and its imported dependencies, whereas existing approaches exhibit an  
𝑂(𝑛)cost with respect to the full repository size. Since𝑛grows much faster than𝑐in real-world  
...
\`\`\`  
\[RQ4\] Summary: Hydra delivers both state-of-the-art accuracy and the lowest, most stable  
latency by anchoring retrieval on dependencies and narrowing the search space to relevant files.  
\`\`\`  
\`\`\`  
6 Threats to Validity  
Our study, while demonstrating the effectiveness of Hydra, is subject to several potential threats  
...
design, also narrows the scope, leaving the effectiveness of Hydra for higher-level tasks (e.g., class  
...
\`\`\`  
7 Conclusion  
In this work, we introduced Hydra, a framework for repository-level code generation that leverages  
...
and DevEval, show that Hydra consistently surpasses baselines, establishes a new state of the  
...
variables, which contributes substantially to performance gains. Error analysis reveals that Hy-  
dra reduces hallucination-related failures and shifts errors toward deeper behavioral validation,  
resulting in more executable and repository-consistent code. Moreover, our time complexity and  
latency evaluation demonstrate that Hydra achieves practical efficiency by restricting retrieval  
...
Generation. In The Twelfth International Conference on Learning Representations, ICLR 2024, Vienna, Austria, May 7-11,  
...
benchmarking neural code generation. IEEE Transactions on Software Engineering 49, 7 (2023), 3675–3691.  
...
recommendation for programming tasks by leveraging crowd knowledge. Empirical Software Engineering 25, 6 (2020),  
4707–4758.  
...
Benchmark for Cross-File Code Completion. In Advances in Neural Information Processing Systems 36: Annual Conference  
on Neural Information Processing Systems 2023, NeurIPS 2023, New Orleans, LA, USA, December 10 \- 16, 2023, Alice Oh,  
Tristan Naumann, Amir Globerson, Kate Saenko, Moritz Hardt, and Sergey Levine (Eds.). \[http://papers.nips.cc/paper\_\](http://papers.nips.cc/paper\_)  
files/paper/2023/hash/920f2dced7d32ab2ba2f1970bc306af6-Abstract-Datasets\_and\_Benchmarks.html  
...
Shengyu Fu, et al.2021. GraphCodeBERT: Pre-training Code Representations with Data Flow. In International  
...
Training and Multi-level Layer Aggregation. In Proceedings of the Third ACM/IEEE International Workshop on NL-Based  
...
In Proceedings of the NeurIPS Datasets and Benchmarks Track (Round 2), NeurIPS. Available in the NeurIPS Datasets  
...
repository into a programming agent environment. In Forty-first International Conference on Machine Learning.  
\[26\]Hung Le, Yue Wang, Akhilesh Deepak Gotmare, Silvio Savarese, and Steven Chu-Hong Hoi. 2022\. CodeRL: Mastering  
Code Generation through Pretrained Models and Deep Reinforcement Learning. In NeurIPS.  
\[27\]Nam Le Hai, Dung Manh Nguyen, and Nghi DQ Bui. 2025\. On the Impacts of Contexts on Repository-Level Code  
Generation. In Findings of the Association for Computational Linguistics: NAACL 2025\. 1496–1524.  
...
Repositories. In Findings of the Association for Computational Linguistics, ACL 2024, Bangkok, Thailand and virtual  
...
A Comprehensive Benchmark for Code Information Retrieval Models. In Proceedings of the 63rd Annual Meeting of the  
...
four different sources. Empirical Software Engineering 28, 3 (2023), 65\.  
...
Completion Systems. In The Twelfth International Conference on Learning Representations, ICLR 2024, Vienna, Austria,  
...
hensive Multilingual Dataset for Advancing Code Understanding and Generation. In Findings of the Association for  
...
CodeLLMs. In The Thirteenth International Conference on Learning Representations. https://openreview.net/forum?id=  
CahIEKCu5Q  
...
and Trends in Information Retrieval 3 (01 2009), 333–389. doi:10.1561/1500000019  
...
SATD and Vulnerability. In 2025 IEEE/ACM 33rd International Conference on Program Comprehension (ICPC). 01–12.  
...
Reinforcement Learning for Repository-Level Code Completion. In 47th IEEE/ACM International Conference on Software  
...
Can Retrieval Augment Code Generation?. In Findings of the Association for Computational Linguistics: NAACL 2025\.  
3199–3214.  
\[49\]Di Wu, Wasi Uddin Ahmad, Dejiao Zhang, Murali Krishna Ramanathan, and Xiaofei Ma. 2024\. Repoformer: Selective  
Retrieval for Repository-Level Code Completion. In Forty-first International Conference on Machine Learning, ICML  
...
2023\. RepoCoder: Repository-Level Code Completion Through Iterative Retrieval and Generation. In Proceedings
