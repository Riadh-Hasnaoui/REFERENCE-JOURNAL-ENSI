...
\#\#\#\# Latest updates: hps://dl.acm.org/doi/10.1145/3663529.
...
\#\#\#\# YINGHAO CHEN, Zhejiang University, Hangzhou, Zhejiang, China

.

\#\#\#\# ZEHAO HU, Zhejiang University, Hangzhou, Zhejiang, China

.

\#\#\#\# CHEN ZHI, Zhejiang University, Hangzhou, Zhejiang, China

.

\#\#\#\# JUNXIAO HAN, Hangzhou City University, Hangzhou, Zhejiang, China

.

\#\#\#\# SHUIGUANG DENG, Zhejiang University, Hangzhou, Zhejiang, China

.

\#\#\#\# JIANWEI YIN, Zhejiang University, Hangzhou, Zhejiang, China
...
\`\`\`  
\`\`\`  
FSE 2024: Companion Proceedings of the 32nd ACM International Conference on the Foundations of Soware Engineering (July 2024\)  
hps://doi.org/10.1145/3663529.  
ISBN: 9798400706585  
\`\`\`  
\#\#\#\# .
...
tion. InCompanion Proceedings of the 32nd ACM International Conference  
...
ACM ISBN 979-8-4007-0658-5/24/  
https://doi.org/10.1145/3663529.
...
\`\`\`  
FSE Companion ’24, July 15–19, 2024, Porto de Galinhas, Brazil Chen et al.  
\`\`\`  
\`\`\`  
Prompt  
\`\`\`  
...
\- •Compile correctnessRuntime correctness  
\- Rule-based repair  
\- LLM-based repair  
\- Test extraction  
\- Test encapsulation  
\- •Adaptive focal contextChain-of-thought  
\- Parsing  
\- Analysis
...
\`\`\`  
\`\`\`  
hatUniTest Core hatUniTest Toolchain  
\`\`\`  
\`\`\`  
Preprocessing  
\`\`\`  
...
\`\`\`  
\`\`\`  
mechanisms, we have developed theChatUniTest Core\[ 10 \]  
to assist researchers and tool builders.  
\`\`\`  
\- We have implemented theChatUniTest Toolchain, which  
...
As shown in Figure 1,ChatUniTest Coreestablishes the overall  
...
\`\`\`  
ChatUniTest: A Framework for LLM-Based Test Generation FSE Companion ’24, July 15–19, 2024, Porto de Galinhas, Brazil  
\`\`\`  
\`\`\`  
...
Inspired by the CAT-LM \[ 23 \], we also buildChatUniTest Mod-  
els\[ 13 \], which provides fine-tuned models for Java test generation  
tasks based onCode Llama\[24\].  
\`\`\`  
...
\`\`\`  
\`\`\`  
mockito method is used incorrectly. Such aswhen()method is used  
...
\`\`\`  
As shown in Figure 1, we build theChatUniTest Toolchainbased  
onChatUniTest Core. The toolchain provides plugins tailored for  
...
Consider the methodequalswithin theStringUtilsclass, as de-  
picted in Figure 2\. This method aims to determine if two given  
CharSequenceobjects,cs1andcs2, are equal.  
\`\`\`  
\`\`\`  
publicstaticbooleanequals(  
CharSequencecs1, CharSequencecs2) {  
if (cs1 \== cs2) {  
returntrue ;  
}  
...
\`\`\`  
\`\`\`  
Figure 2: Buggy methodequalsin Apache Commons.  
\`\`\`  
\`\`\`  
Developers can invoke ChatUniTest to generate unit tests for  
the methodequals. Figure 3 illustrates the generation results. The  
...
\`\`\`  
\`\`\`  
StringBuilderimplementsCharSequencebut does not override the  
equalsmethod from the Object class. As a result, forStringBuilder,  
theequalsmethod checks for reference equality rather than value  
equality. Therefore, the test detects a defect in the focal method.  
\`\`\`  
\`\`\`  
publicclassStringUtilsEqualsTest{  
@Test(timeout \= 8000\)  
...
Porject Information Line Coverage  
...
\[12\] 2023\. https://github.com/ZJU-ACES-ISE/ChatUniTest\_IDEA\_Plugin.  
\[13\] 2024\. https://github.com/ZJU-ACES-ISE/chatunitest-models.  
\[14\]2024. https://github.com/ZJU-ACES-ISE/chatunitest-core/tree/main/docs/  
survey.  
...
for object-oriented software. InProceedings of the 19th ACM SIGSOFT symposium  
...
Graph-based seed object synthesis for search-based unit testing. InProceedings  
...
testing for Java. InCompanion to the 22nd ACM SIGPLAN conference on Object-  
...
2023\. CAT-LM training language models on aligned code and tests. In2023 38th  
...
\`\`\`  
Received 2024-01-29; accepted 2024-04-  
\`\`\`
