...
\#\# Dewu Zheng^1 , Yanlin Wang^1 ∗, Ensheng Shi^2 , Ruikai Zhang^3 , Yuchi Ma^3 , Hongyu Zhang^4 , Zibin Zheng^1 ,^5

(^1) Sun Yat-sen University, Zhuhai, China

\#\#\# zhengdw5@mail2.sysu.edu.cn,{wangylin36, zhzibin}@mail.sysu.edu.cn,

(^2) Huawei Cloud Computing Technologies Co., Ltd., Beijng, China
...
(^5) Zhuhai Key Laboratory of Trusted Large Language Models
...
\`\`\`  
\#\#\# software development \[70\]–\[73\]. Recently, several repository-
...
\#\#\# for evaluation, a situation we term as theevolution-ignored
...
\#\#\# to ensure that the context we provide to LLMs mirrors the

\#\# arXiv:2406.06918v2 \[cs.SE\] 18 Mar 2025

\#\#\# context available to the programmer when writing the target
...
\#\#\# findings:⃝ 1 We find that all the studied LLMs show much
...
\#\#\# deviated programming scenario from reality.⃝ 2 We find that
...
\#\#\# ⃝ 3 We investigate how the performance of LLMs change
...
\#\#\# evolves.⃝^4 We find that, under the evolution-ignored setting,
...
\`\`\`  
\`\`\`  
PromptTe m p l a t e  
Generatethemethod body for:  
\`\`\`  
\`\`\`  
Similarity-based  
Retrieval  
\`\`\`  
\`\`\`  
def register\_instance\_lookup(self, lookup,  
lookup\_name=None):  
if lookup\_name is None:  
lookup\_name \= lookup.lookup\_name  
if "instance\_lookups" not in self.\_\_dict\_\_:  
self.instance\_lookups \= {}  
self.instance\_lookups\[lookup\_name\] \= lookup  
return lookup  
\`\`\`  
\`\`\`  
Here are some context that may help you:  
\`\`\`  
\`\`\`  
\<Input\>  
def register\_lookup(cls, lookup, lookup\_name=None):  
\`\`\`  
\`\`\`  
Committed on Jan 23, 2019  
\`\`\`  
\`\`\`  
\<RetrievedContent\>  
\`\`\`  
...
\`\`\`  
\`\`\`  
Fig. 1\. Example 1: future context leakage.  
\`\`\`  
...
\#\#\# evolves over time. Then, we present two motivating examples^1
...
(^1) The motivating examples are from the real-world project https://github.  
com/django/django
...
\`\`\`  
\`\`\`  
\<Deleted Context\> DeletedonJan7, 2015  
\`\`\`  
\`\`\`  
Committed on May 10, 2013  
\`\`\`  
\`\`\`  
Evolving-ignored  
Repository Base  
...
\`\`\`  
\`\`\`  
if cls.\_migration\_class is None:  
... //code omitted  
\`\`\`  
\`\`\`  
LLMGeneration  
...
\`\`\`  
LLM generates wrong  
code without the help  
of useful context\!\!\!  
\`\`\`  
\`\`\`  
Fig. 2\. Example 2: useful context missing.  
\`\`\`  
...
\#\#\# asfuture context leakageanduse context missing, which we

\#\#\# will illustrate with the following two examples.

\#\#\# Example 1.Figure 1 illustrates thefuture context leakage
...
\#\#\# that islater than the target function. This process is illogical
...
\#\#\# Example 2\. Figure 2 illustrates theuseful context missing
...
def\_get\_nonflat\_cls(cls,kls:Optional\[Type\[\_CosmoT\]\]=None)  
\`\`\`  
\- \>Optional\[Type\[Cosmology\]\]:

\`\`\`  
Detailed docstring  
Find the corresponding non-flat class. The class' bases  
are searched recursively. Parameters  
\----------  
kls : :class:\`astropy.cosmology.Cosmology\` class or None,  
optional If \`None\` (default) this class is searched  
instead of \`kls\`.  
Raises  
\----------  
TypeError If more than one non-flat class is found at the  
same level of the inheritance. This is similar to the  
error normally raised by Python for an inconsistent method  
resolution order. Returns  
\---------  
type A :class:\`Cosmology\` subclass this class inherits  
from that is not a :class:\`FlatCosmologyMixin\` subclass.  
\`\`\`  
\`\`\`  
Basecommit 2ae987ba1160486e36eb315ee0e9d85cdbea  
\`\`\`  
\`\`\`  
Brief docstring Find the corresponding non-flat class.  
\`\`\`  
\`\`\`  
...
\`\`\`  
\`\`\`  
5feceb66ffc8 astropy/astropy  
5.1 Dependencylevel standalone  
\`\`\`  
\`\`\`  
Ta s kInstanceOverview  
\`\`\`  
\`\`\`  
Fig. 3\. Task instance overview.  
\`\`\`  
...
\#\#\# Owner/Repofield helps identify the GitHub repository from

\#\#\# which the selected function originates, while theVersion

\#\#\# field indicates the version of the repository according to the

\#\#\# implementation time of this function.Base commitrefers
...
\#\#\# before the target function was implemented. TheFunction

\#\#\# declarationof the target function will be provided to

\#\#\# the LLM as part of the prompt, along with its corresponding

\#\#\# docstring. Since the docstring styles of functions in pragmatic
...
\#\#\# The Brief docstringprovides a concise summary of

\#\#\# the functionality of the target function, while theDetailed

\#\#\# docstringprovides a thorough description of the function’s
...
\#\#\# high-quality real-world projects from a list of the top 5,

\#\#\# most downloaded PyPI libraries^2. Subsequently, we deploy the
...
\#\#\# developer API^3. For Java, as widely recognized projects
...
\- The PR must introduce at least one or more new tests,
...
\#\#\# tators with more than five years of programming experience

\`\`\`  
High Quality Projects  
\`\`\`  
\`\`\`  
PyPI downloading leaderboard  
\`\`\`  
\`\`\`  
Project Selection  
\`\`\`  
\`\`\`  
Top-200 highly stared  
\`\`\`  
\`\`\`  
...
\`\`\`  
\`\`\`  
Pull Request Crawling 1\. Installs successfully  
\`\`\`  
\`\`\`  
More than 10,000 PRs About 2,000 PRs 400 PRs  
\`\`\`  
\`\`\`  
Human Annotation  
\`\`\`  
2\. Rewritesrequirement  
1\. Dependency-levelclassify

\`\`\`  
Attribute-based Filtering  
\`\`\`  
\`\`\`  
2.Adds new functions  
\`\`\`  
1\. Merged status  
3\. Contributes tests
...
\`\`\`  
\`\`\`  
400 Programming tasks  
\`\`\`  
\`\`\`  
Fig. 4\. HumanEvo construction pipeline.  
\`\`\`  
...
\#\#\# that should have been present but might have been deleted or

\#\#\# updated during the project’s evolution. This undoubtedly leads
...
\#\#\# function, the results of the two executions should show a “fail-

\#\#\#\#\# TABLE I

\#\#\#\#\# DEPENDENCY LEVEL INHUMANEVO.
...
\#\#\# by processing PRs appropriately, we can further determine

\#\#\# whether the target functions are covered by the project’s testing
...
\#\#\# model. There are four types of context sources: local file (the

\#\#\# file where the target code resides), import file (files imported
...
\#\#\# Platinum 8336C @ 2.30GHzE CPUs, and 8 NVIDIA A
...
\#\#\# source is the latest version of the project) andEvolution-
...
\#\#\# approach, and Table III shows the performance of static

\#\#\#\#\# TABLE II

\#\#\#\#\# RETRIEVAL-BASED CONTEXT ACQUISITION APPROACH:MODEL

\#\#\#\#\# PERFORMANCE UNDER EVOLUTION-AWARE(HUMANEVO)AND
...
\#\#\# evolution-ignored setting. However, due to the significantly

\#\#\#\#\# TABLE III

\#\#\#\#\# STATIC ANALYSIS BASED PROJECT CONTEXT ACQUISITION APPROACHES:MODEL PERFORMANCE UNDER EVOLUTION-AWARE(HUMANEVO)AND

\#\#\#\#\# EVOLUTION-IGNORED(EI)SETTINGS.

\`\`\`  
Data Model local local+import local+sibling local+sim  
...
\`\`\`  
\`\`\`  
CodeLlama-7B 27.0% 24.0%(↓ 11 .1%) 28.0% 25.0%(↓ 10 .7%) 28.5% 25.0%(↓ 12 .3%) 29.0% 25.0%(↓ 13 .8%)  
...
\`\`\`  
\`\`\`  
CodeLlama-7B 16.5% 8.0%(↓ 51 .5%) 15.0% 6.5%(↓ 56 .7%) 12.0% 6.0%(↓ 50 .0%) 13.5% 6.5%(↓ 51 .9%)  
...
\`\`\`  
\`\`\`  
evo0evo1evo2evo3evo4evo5evo6evo7evo8evo9evo  
\`\`\`  
\`\`\`  
30  
\`\`\`  
...
\`\`\`  
\`\`\`  
Numbers of successful instances  
\`\`\`  
\`\`\`  
44  
\`\`\`  
...
\#\#\# the accuracy of LLM in generating both intra-class and inter-

\#\#\# class functions has noticeably declined. The decline is more
...
\#\#\# LLM’s performance under the evolution-ignored setting.

\#\#\#\#\# TABLE IV

\#\#\#\#\# BREAK DOWN ANALYSIS. EAANDEISTAND FOR EVOLUTION-AWARE
...
\`\`\`  
\`\`\`  
CodeLlama-7B 32.8%31.1% (↓ 5 .0%) 1.6% 1.6%  
...
\`\`\`  
\`\`\`  
CodeLlama-7B 27.2%17.5% (↓ 35 .7%) 10.6%5.3% (↓ 50 .0%)  
...
\#\#\# LLMs deviates from the actual performance in the evo
...
\#\#\# RQ2 Summary: In the evolution-ignored setting, as the
...
\#\#\# of all LLMs in actual development scenarios, which may
...
\`\`\`  
\`\`\`  
Detailed 30.5% 23.0% 9.5% 7.0%  
Brief 30.0% 22.5% 8.5% 5.0%  
\`\`\`  
\`\`\`  
CodeLlama-34B Detailed 31.5% 24.0% 14.5% 10.0%  
...
\`\`\`  
\`\`\`  
Detailed 31.0% 25.0% 15.5% 11.5%  
Brief 31.0% 24.0% 13.5% 9.0%  
\`\`\`  
\`\`\`  
GPT-3.5-Turbo Detailed 32.5% 25.5% 18.0% 13.0%  
Brief 31.5% 24.5% 14.5% 11.0%  
GPT-4 Detailed 34.5% 26.5% 20.5% 14.5%  
...
\#\#\# collects data from 25 real-world repositories. It also ensures

\#\#\# that the distribution of programming tasks (with or without
...
\#\#\# results.A^3 \-CodGen \[7\] first constructs a repository knowledge

\#\#\# base covering knowledge of third-party libraries and extracts

\#\#\# information on mobile and code context. During the code

\#\#\# generation process, LLM first generates an initial version of
...
\#\#\# In addition to these models, there are many other excellent

\#\#\# models used in the field of code generation \[17\], \[18\], \[29\]–
...
Generating code by retrieving the docs,” inThe Eleventh International  
...
“a^3 \-codgen: A repository-level code generation framework for code  
...
J. Bitton, M. Bhatt, C. C. Ferrer, A. Grattafiori, W. Xiong, A. D ́efossez,  
...
reliable benchmark for data science code generation,” inInternational  
...
level code auto-completion systems,” inThe Twelfth International Con-  
ference on Learning Representations, 2024\.  
\[15\] J. Li, G. Li, Y. Zhao, Y. Li, Z. Jin, H. Zhu, H. Liu, K. Liu, L. Wang,  
Z. Fanget al., “Deveval: Evaluating code generation in practical software  
...
code infilling and synthesis,” inThe Eleventh International Conference  
on Learning Representations, 2023\.  
...
to code in programmatic context,” in2018 Conference on Empirical  
...
with evol-instruct,” inThe Twelfth International Conference on Learning  
Representations, 2024\.  
\[30\] F. Christopoulou, G. Lampouras, M. Gritta, G. Zhang, Y. Guo, Z. Li,  
Q. Zhang, M. Xiao, B. Shen, L. Li, H. Yu, L. Yan, P. Zhou, X. Wang,  
Y. Ma, I. Iacobacci, Y. Wang, G. Liang, J. Wei, X. Jiang, Q. Wang, and

\`\`\`  
Q. Liu, “Pangu-coder: Program synthesis with function-level language  
modeling,” 2022\.  
...
more effective,” inProceedings of the 45th international ACM SIGIR  
...
Available: https://aclanthology.org/2022.naacl-main.  
...
learning for dense text retrieval,” inInternational Conference on Learn-  
...
B. Xiang, “Multi-lingual evaluation of code generation models,” inThe  
...
synthesis,” inProceedings of the 44th International Conference on  
Software Engineering, 2022, pp. 1219–1231.  
\`\`\`

\[47\] D. Zan, B. Chen, D. Yang, Z. Lin, M. Kim, B. Guan, Y. Wang,  
W. Chen, and J.-G. Lou, “Cert: Continual pre-training on sketches for  
library-oriented code generation,” inProceedings of the Thirty-First  
...
https://doi.org/10.24963/ijcai.2022/  
...
“When language model meets private library,” inFindings of the  
...
\[Online\]. Available: https://aclanthology.org/2022.findings-emnlp.
...
N. Muennighoff, M. Mishra, A. Gu, M. Deyet al., “Santacoder: don’t  
...
“Planning with large language models for code generation,” inThe  
Eleventh International Conference on Learning Representations, 2023\.  
\[55\] L. Voinea, A. Telea, and J. J. van Wijk, “Cvsscan: visualization  
of code evolution,” inProceedings of the 2005 ACM Symposium  
on Software Visualization, ser. SoftVis ’05. New York, NY, USA:  
Association for Computing Machinery, 2005, p. 47–56. \[Online\].  
Available: https://doi.org/10.1145/1056018.  
...
and growth in large libre software projects,” inIWPSE, 2005, pp. 165–  
174\.  
...
projects using github,” in2015 IEEE/ACM 37th IEEE international  
conference on software engineering, vol. 1\. IEEE, 2015, pp. 574–585.  
\[63\] M. M. Rahman and C. K. Roy, “An insight into the pull requests  
of github,” inProceedings of the 11th working conference on mining  
...
\`\`\`  
\[68\] H. N. Phan, H. N. Phan, T. N. Nguyen, and N. D. Q. Bui, “Repohy-  
...
code is all you need,” inInternational Conference on Machine Learning,  
2023\.  
...
models,” 2023\. \[Online\]. Available: https://arxiv.org/abs/2303.  
