...
\# MULTI-AGENT COLLABORATIVEFRAMEWORK
...
Liyang Zhou^6 ,Chenyu Ran^1 ,Lingfeng Xiao^1 ,^7 ,Chenglin Wu^1 †,Jurgen Schmidhuber ̈^2 ,^8  
\`\`\`  
...
functionality. Remarkably, in our experimental evaluations, MetaGPT achieves a100%task com-  
...
\#\# 3 METAGPT: A META-PROGRAMMINGFRAMEWORK
...
work, and illustrates how to organize a multi-agent system within the context of SOPs. Sec. 3\.  
...
\#\#\# 3.1 AGENTS INSTANDARDOPERATINGPROCEDURES
...
To address this challenge, a viable approach is to store information in a globalmessage pool. As  
shown in Figure 2 (left), we introduce a shared message pool that allows all agents to exchange  
messages directly. These agents not onlypublishtheir structured messages in the pool but also access  
...
(^2) https://en.wikipedia.org/wiki/Chinesewhispers
...
Evaluation Metrics For HuamnEval and MBPP, we follow the unbiased version ofPass @kas  
...
proves thePass @kin the HumanEval benchmark compared to GPT-4. It achieves 85.9% and 87.7%

(^3) https://en.wikipedia.org/wiki/Read–eval–printloop
...
sion, it also significantly outperforms ChatDev. Although MetaGPT requires more tokens (24,  
...
(A)Executability 2.25 3.67 3\.  
(B)Cost\#1: Running Times (s) 762 503 541  
(B)Cost\#2: Token Usage 19,292 24,613 31,  
(C)Code Statistic\#1: Code Files 1.9 4.6 5\.  
(C)Code Statistic\#2: Lines of Code per File 40.8 42.3 49\.  
(C)Code Statistic\#3: Total Code Lines 77.5 194.6 251\.  
(D)Productivity 248.9 126.5 124\.  
(E)Human Revision Cost 2.5 2.25 0\.  
\`\`\`  
\#\#\# 4.3 CAPABILITIESANALYSIS
...
feedback into MetaGPT leads to a significant improvement of 4.2% and 5.4% inPass @1on Hu-  
...
attempt to regulate LLM-based multi-agent frameworks. See also theoutlook (Appendix A).
...
made the most significant code contributions to it, and advised this project. Jurgen Schmidhuber, ̈  
...
R.M. Belbin.Team Roles at Work. Routledge, 2012\. URLhttps://books.google.co.uk/  
...
Xinyun Chen, Chang Liu, and Dawn Song. Execution-guided neural program synthesis. InICLR,  
2018\.
...
Schutze, and Yoav Goldberg. Measuring and improving consistency in pretrained language mod- ̈  
els.TACL, 2021\.
...
John McCarthy. History of lisp. InHistory of programming languages. 1978\.

Ansong Ni, Srini Iyer, Dragomir Radev, Veselin Stoyanov, Wen-tau Yih, Sida Wang, and Xi Victoria  
Lin. Lever: Learning to verify language-to-code generation with execution. InICML, 2023\.
...
Adi, Jingyu Liu, Tal Remez, Jer ́ ́emy Rapin, et al. Code llama: Open foundation models for code.  
arXiv preprint, 2023\.
...
J. Schmidhuber. A self-referential weight matrix. InProceedings of the International Conference  
on Artificial Neural Networks, Amsterdam, pp. 446–451. Springer, 1993a.

J. Schmidhuber. Godel machines: self-referential universal problem solvers making provably ̈  
...
J. Schmidhuber. Godel machines: Fully self-referential optimal universal self-improvers. In B. Go- ̈  
...
J. Schmidhuber. Ultimate cognitiona la\` Godel. ̈ Cognitive Computation, 1(2):177–193, 2009\.

Jurgen Schmidhuber. ̈ Evolutionary principles in self-referential learning, or on learning how to  
learn: the meta-meta-... hook. PhD thesis, 1987\.

Jurgen Schmidhuber. A ‘self-referential’weight matrix. In ̈ ICANN’93: Proceedings of the Interna-  
tional Conference on Artificial Neural Networks Amsterdam, The Netherlands 13–16 September  
1993 3, 1993b.

Jurgen Schmidhuber. On learning to think: Algorithmic information theory for novel combinations ̈  
of reinforcement learning controllers and recurrent neural world models.arXiv preprint, 2015\.

Jurgen Schmidhuber, Jieyu Zhao, and Nicol N Schraudolph. Reinforcement learning with self- ̈  
modifying policies. InLearning to learn. 1998\.
...
Kourosh Darvish, Alan Aspuru-Guzik, Florian Shkurti, and Animesh Garg. Errors are useful ́  
...
Michael Wooldridge and Nicholas R. Jennings. Pitfalls of agent-oriented development. InPro-  
ceedings of the Second International Conference on Autonomous Agents, 1998\. URLhttps:  
...
Mingchen Zhuge, Haozhe Liu, Francesco Faccio, Dylan R Ashley, Robert Csord ́ ́as, Anand  
...
\#\#\# A.1 SELF-IMPROVEMENTMECHANISMS
...
system by improving each individual in the company. We first establish ahandover feedbackaction  
...
prompt. The summarized information is stored inlong-term memorysuch that it can be inherited  
by future constraint prompt updates. When initiating a new project, each agent starts with areact  
...
(^4) \[http://beta.deepwisdom.ai\](http://beta.deepwisdom.ai)
...
\`\`\`  
\#\#\# B.2 METAGPTDEVELOPMENT PROCESS
...
\`\`\`  
Figure 7: The quadrant chart for the “Drawing App” generated by theproduct manageragent.
...
\`\`\`  
Figure 9: The program call flow for the “Drawing App” generated by thearchitectagent.  
\`\`\`  
Tasks
...
30 ("color\_picker.py", "Implements the color selection interface."),  
31 ("file\_manager.py", "Implements the file saving and opening functionality.")  
32 \]  
33 ‘‘‘  
\`\`\`
...
16 The ’Pillow’ library is used for handling image files. It is used in the ’file\_manager.py’ for  
...
\`\`\`  
Generated code (colorpicker.py)  
\`\`\`  
\`\`\`  
1 import tkinter as tk  
2 from tkinter import colorchooser  
3 class ColorPicker:  
4 def \_\_init\_\_(self, root: tk.Tk, color: str \= ’black’):  
5 self.frame \= tk.Frame(root)  
6 self.color \= color  
7  
8 self.color\_button \= tk.Button(self.frame, text=’Color’, command=self.select\_color)  
9 self.color\_button.pack(fill=’x’)  
10  
11 def select\_color(self):  
...
15  
16 def test\_initial\_color(self):  
17 \# Test the default color during initialization  
18 self.assertEqual(self.cp.get\_color(), ’black’)  
19  
20 def test\_set\_and\_get\_color(self):  
21 \# Test setting a new color and retrieving it  
22 new\_color \= ’\#ffffff’ \# white color  
23 self.cp.color \= new\_color  
24 self.assertEqual(self.cp.get\_color(), new\_color)  
25  
26  
...
\#\#\# C.1 DETAILS OF THESOFTWAREDEVDATASET
...
Model Open sourceTime(/s)\# LinesExecutabilityRevisions  
...
\`\`\`  
Impact of Instruction Levels (High-levelv.s.Detailed Instructions) Does the variation in the  
...
\`\`\`  
Model \# WordTime(/s) Token usage \# LinesExecutability Productivity Reversions  
...
\#\# D LIMITATION ANDETHICSCONCERNS
...
thearchitectagent (zoom in for a better view).
...
architectagent (zoom in for a better view).
...
Additional results of pure MetaGPT w/o feedback on SoftwareDev.

Averages (Avg.) of 70 tasks are calculated and 10 randomly selected tasks are
...
$ 1.01  
\`\`\`  
1\. PNG file miss-ing 2\. Compile bugfixes
...
\`\`\`  
\`\`\`  
pile bug fixes 3.pygame.surface notinitialize  
\`\`\`  
\`\`\`  
3  
\`\`\`  
...
$ 1.51  
\`\`\`  
1\. dependency er-ror 2\. ModuleNot-FoundError
...
$ 1.27  
\`\`\`  
1\. tensorflow ver-sion error 2\. modeltraining method notimplement
...
$ 1.27  
\`\`\`  
1\. dependency er-ror 2\. URL 403 er-ror
...
$ 0.92  
\`\`\`  
1\. dependency er-ror 2\. missing mainfunc.
...
\`\`\`  
\`\`\`  
0.51 (only consideritem scored 2, 3 or4)  
\`\`\`  
\`\`\`  
3.36  
\`\`\`
