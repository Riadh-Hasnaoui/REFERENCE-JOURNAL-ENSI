...
\#\#\#\# Latest updates: hps://dl.acm.org/doi/10.1145/3611643.

\`\`\`  
..  
RESEARCH-ARTICLE  
\`\`\`  
\#\# RAP-Gen: Retrieval-Augmented Patch Generation with CodeT5 for

\#\# Automatic Program Repair

\#\#\#\# WEISHI WANG, Nanyang Technological University, Singapore City, Singapore

.

\#\#\#\# YUE WANG, Salesforce.com, Inc., San Francisco, CA, United States

.

\#\#\#\# SHAFIQ JOTY, Salesforce.com, Inc., San Francisco, CA, United States

.

\#\#\#\# STEVEN C.H. HOI, Salesforce.com, Inc., San Francisco, CA, United States
...
Soware Engineering Conference and  
...
\`\`\`  
\`\`\`  
ESEC/FSE 2023: Proceedings of the 31st ACM Joint European Soware Engineering Conference and Symposium on the Foundations of Soware Engineering (November 2023\)  
hps://doi.org/10.1145/3611643.  
ISBN: 9798400703270  
\`\`\`  
\#\#\#\# .

\# RAP-Gen: Retrieval-Augmented Patch Generation with CodeT
...
propose a novelRetrieval-AugmentedPatchGeneration frame-  
...
ACM ISBN 979-8-4007-0327-0/23/12... $15.  
https://doi.org/10.1145/3611643.

\`\`\`  
boosting the accuracy of T5-large on TFix from 49.70% to 54.15%  
...
bugging;•Computing methodologies→Natural language  
processing.
...
Program Repair. InProceedings of the 31st ACM Joint European Software  
Engineering Conference and Symposium on the Foundations of Software Engi-  
neering (ESEC/FSE ’23), December 3–9, 2023, San Francisco, CA, USA.ACM,  
New York, NY, USA, 13 pages. https://doi.org/10.1145/3611643.  
\`\`\`  
\#\#\# 1 INTRODUCTION
...
19 , 34 , 51 , 71 , 73 \]. They often search for repairs based on thefix  
patternsmined via manual heuristic rules \[ 24 , 51 , 60 \] or redundancy-  
based techniques \[ 12 , 19 , 34 – 36 , 73 \]. The latter group of approaches  
make aredundancy assumption\[ 74 \] that the fixed patch can often  
...
nologies, numerous deep learning (DL)-based APR approaches \[ 4 , 7 ,  
\`\`\`

\`\`\`  
ESEC/FSE ’23, December 3–9, 2023, San Francisco, CA, USA Weishi Wang, Yue Wang, Shafiq Joty, and Steven C.H. Hoi  
\`\`\`  
\`\`\`  
Error msg : Expected an object to be thrown.  
Buggy code :  
if (\_.isUndefined(exp)) {throw "expected a value, got undefined  
instead";}  
\`\`\`  
\`\`\`  
Before fix :  
\`\`\`  
evaluate: function() { throw "undefined for looping expressions"; (^)  
}  
(^) \*\*After fix:\*\*  
evaluate: function() { throw \*\*new Error\*\* ("undefined for looping  
expressions");  
}  
if (\_.isUndefined(exp)) {throw \*\*new Error\*\* ("expected a value, got  
undefined instead");}  
\*\*You face a bug One bug fix example in codebase  
Retrieve  
Fix\*\*  
...
top relevantbug-fix pairas a guiding fix pattern for a buggy patch  
...
the string with an “Error” object for the “throw” statement, which  
...
searches for a relevant bug fix pattern and then pass it to the CodeT  
...
large-scale pretrained language models for code.  
For the retrievers, we propose a hybrid approach that accounts  
for both lexical and semantic matching through sparse (BM25 \[ 56 \])  
and dense (DPR \[ 23 \]) retrieval based on the raw source code. We  
employ CodeT5’s encoder as our dense DPR retriever and propose  
...
\- We propose a novel retrieval-augmented patch generation  
...
\- We present a hybrid patch retriever for fix pattern mining  
...
\- We propose to adapt a generic pretrained code-aware lan-  
...
\- We extensively evaluate RAP-Gen on three APR benchmarks  
    ...
    (49.70→54.15 on exact match accuracy and 69.30→78.80 on  
    ...
    24.80 and 15.84 over CodeT5’s 21.61 and 13.96 for the small  
    and medium subsets. On Defects4J, RAP-Gen achieves new  
    SoTA performance, repairing 15 more bugs (110→125) with  
    perfect FL and 6 more bugs (68→74) without perfect FL  
    than the previous SoTA models.

\`\`\`  
RAP-Gen: Retrieval-Augmented Patch Generation with CodeT5 for Automatic Program Repair ESEC/FSE ’23, December 3–9, 2023, San Francisco, CA, USA  
\`\`\`  
\`\`\`  
Fix  
\`\`\`  
...
tests. The search strategy has been further extended to adoptfix  
patternsmined using redundancy-based techniques \[ 12 , 19 , 26 , 34 –  
36 , 73 \]. These approaches make aredundancy assumption\[ 74 \] that  
...
sequence NMT problem \[ 58 \], which is to translate a buggy program

\`\`\`  
into a fixed version. Various neural architectures have been ex-  
...
e.g., one of the largest one Defects4J only contains less than 1000  
\`\`\`
...
the commit comments contain some keywords such as “repair” and  
“fix”. We consider all these types of APR use cases in this work.
...
to the finetuned T5’s 59% on a random sample of 200 instances from  
...
their APIs, e.g., the Davinci version costs $0.02 for every 1K tokens^1.
...
consists of three stages: 1\) a patch retriever training stage to learn  
...
Section 3.2, followed by detailing thehybrid patch retrieverin Sec-  
tion 3.3 and theretrieval-augmented patch generatorin Section 3.4.

\#\#\# 3.1 Task Formulation

\`\`\`  
LetD \= {(𝑋𝑖,𝑌𝑖)}𝑖|D|= 1 be a program repair dataset consisting  
of|D|bug-fix pairs(𝑋𝑖,𝑌𝑖), where𝑋𝑖and𝑌𝑖are the𝑖-th buggy  
...
C={(𝐵𝑗,𝐹𝑗)}|C|𝑗= 1 , where(𝐵𝑗,𝐹𝑗)denotes the𝑗-th previous bug-fix  
pair. Given a buggy program patch𝑋𝑖inD, a retriever retrieves  
the most relevant bug-fix pair(𝐵𝑗,𝐹𝑗)in the codebaseCbased on a  
relevance scoring function𝑓𝜙(𝑋𝑖,𝐵𝑗)parameterized by𝜙. Then the  
original input sequence𝑋𝑖is augmented with the retrieved bug-fix  
pair to form a new input sequence𝑋ˆ𝑖=𝑋𝑖⊕𝐵𝑗⊕𝐹𝑗, where⊕  
denotes the concatenation operation. The sequence-to-sequence  
(seq2seq) generator then generates𝑌𝑖from𝑋ˆ𝑖in an autoregressive  
manner. Formally, we aim to learn the following probability with  
the patch seq2seq generator parameterized by𝜃:  
\`\`\`  
\`\`\`  
𝑃𝜃(𝑌𝑖|𝑋ˆ𝑖)=  
\`\`\`  
...
𝑘= 1  
\`\`\`  
\#\#\#\#\# 𝑃𝜃(𝑌𝑖,𝑘|𝑋ˆ𝑖,𝑌𝑖, 1 :𝑌𝑖,𝑘− 1 )

\`\`\`  
where𝑌𝑖, 1 :𝑌𝑖,𝑘− 1 is the previous sequence before the𝑘-th token  
and𝑛denotes the number of tokens in the target sequence𝑌𝑖.  
Note that we regard the external codebaseCas a non-parametric  
...
as a latent variable𝑍𝑗=(𝐵𝑗,𝐹𝑗), which is approximated by top-  
...
\`\`\`  
where𝑍∗𝑗is the top-1 retrieved output from the retriever𝑃𝜙(𝑍𝑗|𝑋𝑖).  
We adopt such top-1 approximation as marginalization over large𝑘  
\`\`\`
...
We also tried to employ top-𝑘(𝑘 \= 2 , 3 , 5 ) with the Fusion-in-  
Decoding or FiD method \[ 18 \] but did not observe a salient per-  
formance improvement.

\#\#\# 3.2 Revisiting CodeT
...
commodate arbitrary tokens (commonly known asopen vocabulary  
...
function𝑓𝜙(𝑋𝑖,𝐵𝑗)to compute the relevance between the (query)  
bug𝑋𝑖inDand a previous (key) bug𝐵𝑗in the codebaseC. As  
...
each other for more robust text retrieval.

\`\`\`  
Lexical-based Retriever.We employ BM25 \[ 56 \], a well-known  
...
patch𝑋𝑖and a candidate patch𝐵𝑗. The computed similarity score is  
represented as𝑓𝜙(𝑋𝑖,𝐵𝑗)=𝐵𝑀 25 (𝑋𝑖,𝐵𝑗). As a sparse term-based  
...
code𝐵𝑗as the query and the corresponding fixed code𝐹𝑗as the  
...
We use a shared DPR to separately encode the query patch𝑋𝑖in  
Dand a candidate patch𝐵𝑗inCas𝐶𝐿𝑆𝑋𝑖and𝐶𝐿𝑆𝐵𝑗, respectively.  
Then the similarity is computed by the inner product between these  
two patch representations as the following:  
\`\`\`  
\`\`\`  
𝑓𝜙(𝑋𝑖,𝐵𝑗)=𝑠𝑖𝑚(𝑋𝑖,𝐵𝑗)=\[𝐶𝐿𝑆𝑋𝑖\]𝑇\[𝐶𝐿𝑆𝐵𝑗\]  
\`\`\`  
\`\`\`  
For training the DPR retriever, we leverage thein-batch negatives  
...
\`\`\`  
𝑗∈M,𝑗≠𝑖exp(𝑠𝑖𝑚(𝐵𝑖,𝐹𝑗))  
\`\`\`  
\`\`\`  
whereMis the current minibatch and𝑁denotes the number of  
...
example will have|M|− 1 negative samples. Note that we do not  
adopt the hard negative mining strategy as in \[ 23 \] due to the noisy  
nature of the training data.  
In the inference stage, given a query buggy patch𝑋𝑖, the DPR  
retrieves a relevant bug-fix pair(𝐵𝑗,𝐹𝑗)by computing the similarity  
between𝑋𝑖(query) and𝐵𝑗(key). We also tried to base on the  
similarity between𝑋𝑖and𝐹𝑗but it did not yield better results.  
\`\`\`  
\`\`\`  
...
the BM25 and DPR. The similarity score is computed as𝑓𝜙(𝑋𝑖,𝐵𝑗)=  
𝑠𝑖𝑚(𝑋𝑖,𝐵𝑗)+𝜆BM25(𝑋𝑖,𝐵𝑗), where𝜆is a weight to balance the  
...
fix pair(𝐵𝑗,𝐹𝑗)as a fix pattern to guide the patch generator for bug  
fixing. The hybrid retriever is expected to be more robust compared  
to retrievers that rely only on either lexical or semantic information.  
\`\`\`
...
\`\`\`  
Error Information :  
fix guard-for-in The body of a for-in should be wrapped in an if  
...
\`\`\`  
Buggy Code :  
var e, ext; for (e in data. installs ) {  
ext \= data. installs \[e\];  
Fixed Code :  
var e, ext; for (e in data.installs) {  
...
\`\`\`  
\`\`\`  
if (endIndex \< 0 || (endIndex \== 0 && startIndex \< endIndex) ) {  
\`\`\`  
\`\`\`  
...
\`\`\`  
\`\`\`  
private synchronized void METHOD\_1 ( java.lang.String VAR\_1 ) {  
if ( VAR\_1 \!= null ) {  
TYPE\_1. i ( VAR\_2 , ( STRING\_1 \+ VAR\_1 ) ) ;  
VAR\_3 \= VAR\_1 ; } }  
private synchronized void METHOD\_1 ( java.lang.String VAR\_1 ) {  
VAR\_3 \= VAR\_1 ;  
}  
...
\`\`\`  
Buggy Code :  
public void METHOD\_1 ( java.lang.String VAR\_1 ) {  
TYPE\_1. i ( VAR\_2 , ( ( STRING\_1 \+ VAR\_1 ) \+ STRING\_2 ) ) ;  
VAR\_3 \= VAR\_1 ; }  
Fixed Code :  
public void METHOD\_1 ( java.lang.String VAR\_1 {  
VAR\_3 \= VAR\_1 ; }  
...
As shown in Fig. 2 2, given a buggy patch𝑋𝑖, we search for a top  
relevant fix pattern(𝐵𝑗,𝐹𝑗)and pass it to the patch generator to  
generate a fixed code patch𝑌𝑖. We adopt a simple yet effective  
strategy to augment𝑋𝑖into𝑋ˆ𝑖=𝑋𝑖⊕𝐵𝑗⊕𝐹𝑗via appending the  
...
Training.We prepare the retrieval-augmented input to CodeT  
patch generator as𝑋ˆ𝑖= “\[CLS\]X𝑖\[BUG\]𝐵𝑗\[FIX\]𝐹𝑗”, where\[BUG\]  
and\[FIX\]are special tokens to separate the retrieved bug-fix pair  
from the buggy patch. CodeT5’s encoder takes𝑋ˆ𝑖as input and emits  
the fixed patch𝑌𝑖from its decoder in an autoregressive manner  
(see Section 3.1). We consider two settings of the buggy patch𝑋𝑖  
...
error line, the buggy patch will be augmented to “error information  
\[SEP\]𝑋𝑖” to incorporate error information to help fix the bugs. To  
train the patch generator, we adoptteacher forcing\[ 63 \] to minimize  
the cross entropy lossL𝑐𝑒over all training instances defined as:
...
evaluation sets are not overlapped with the training set as well.  
\`\`\`  
\`\`\`  
Inference with Beam Search.During inference, as shown in  
...
predictions is determined by the beam sizeB. Concretely, at each  
decoding timestep, the beam search selects the mostBpromising  
...
analyzer ESLint^2 \[ 62 \]. In addition to error types, it provides rich  
...
TFix Original 84,846 9,454 10,  
TFix Deduplicated 84,673 9,423 10,  
Code Refinement Small 46,680 5,835 5,  
Code Refinement Medium 52,364 6,545 6,  
Defects4J v1.2 \- \- 388  
Defects4J v2.0 \- \- 430  
\`\`\`  
...
aduplication issueinside each data split and between data splits.  
...
Evaluation Metrics.We reportExact Match(EM) accuracy and  
BLEU-4score to evaluate program repair performance follow-  
...
we further employError Removalmetric following \[ 4 \] to take  
...
public GitHub Archive^3 between March 2011 and October 2017\.  
They use Google BigQuery APIs to identify all Java commits having  
a message containing the patterns: (“fix” or “solve”) and (“bug”  
or “issue” or “problem” or “error”) to ensure the quality of the  
...
number of tokens, i.e., \# of code tokens≤50 for the small set and  
50 \<\# of code tokens≤100 for the medium set. Since the bug  
...
Model  
EM w/ spaces EM w/o spaces  
Avg. W. Avg. Avg. W. Avg.  
Naive Copy 0.00 0.00 0.00 0\.  
SequenceR 17.90 \- \- \-  
CoCoNuT 11.70 \- \- \-  
T5-small 44.46 44.44 44.52 44\.  
T5-base 48.54 47.63 48.72 47\.  
T5-large 49.33 49.65 49.35 49\.  
CodeT5-small 47.14 46.35 50.22 50\.

\- error-information 45.97 45.80 49.26 49\.  
CodeT5-base 50.88 49.42 54.30 53\.  
\- error-information 46.88 47.17 50.36 51\.
...
bugs can be found in Fig. 3(c), where “-” denotes a buggy line to be  
fixed and “+” represents the correct fix committed from a developer.  
...
1,039,873 synthetic bug-fix Java patches. We use a subset of 830,  
training data that is available online.^4 For testing, we follow their  
...
\`\`\`  
(^4) https://github.com/ASSERT-KTH/SelfAPR/tree/main/dataset
...
\`\`\`  
Model EM BLEU-  
T5-large (TFix) 49.58 76\.  
CodeT5-base 53.46 78\.  
RAP-Gen 54.15 79\.  
\`\`\`  
...
epochs using the contrastive lossLinfoNCEusing a batch size of 64  
and a learning rate of 2e-5. We finetune RAP-Gen generator for  
30 epochs using a sequence generation lossL𝑐𝑒using a batch size  
...
library^5 of BM25, which can be efficiently trained on CPU within  
...
\#\#\# 5.1 RQ1: Comparative Study with DL-based
...
\`\`\`  
(^5) https://pypi.org/project/rank-bm  
Table 4: Performance breakdown on 52 error types on TFix.  
Error Type \#Samples T5-large RAP-GenError Type \#SamplesT5-large RAP-Gen  
no-new-symbol 10 100.00 100.00 no-extra-bind 674 70.59 73\.  
no-compare-neg-zero 13 0.00 0.00 no-case-declarations 723 58.90 67\.  
no-ex-assign 40 25.00 25.00 no-fallthrough 743 76.00 77\.  
for-directionno-unsafe-finally 5063 40.0042.86 60.0014.29 no-inner-declarationsno-array-constructor 830980 38.1086.73 46.4385.  
use-isnan 71 37.50 25.00 no-constant-condition 1,251 48.78 54\.  
no-class-assign 111 41.67 50.00 generator-star-spacing 1,396 67.86 72\.  
no-dupe-class-members 117 8.33 8.33 no-extra-boolean-cast 1,458 54.11 58\.  
no-func-assign 147 46.67 60.00 no-cond-assign 1,472 45.21 47\.  
no-empty-patternno-unused-labels 178187 27.7852.63 44.4463.16 no-process-exitno-empty 1,5142,055 32.8926.70 37.5031.  
no-duplicate-case 195 65.00 60.00 no-dupe-keys 2,181 53.42 55\.  
getter-return 203 52.38 61.90 prefer-spread 2,466 45.08 45\.  
no-sparse-arrays 237 25.00 45.83 no-useless-escape 2,920 35.15 40\.  
no-const-assign 277 35.71 42.86 no-console 3,067 73.62 73\.  
no-global-assignno-new-wrappers 318360 59.3827.78 68.7538.89 guard-for-inno-throw-literal 3,2314,075 41.9872.06 45.3774.  
no-this-before-super 413 47.62 69.05 no-debugger 4,164 94.48 94\.  
no-unsafe-negation 423 72.09 76.74 prefer-rest-params 4,534 35.68 43\.  
require-yield 429 72.09 76.74 no-unreachable 4,725 63.85 64\.  
no-extend-native 443 31.11 26.67 no-extra-semi 5,969 82.61 83\.  
no-new-object 446 71.11 66.67 no-redeclare 6,381 49.45 59\.  
no-callerconstructor-super 446464 20.0059.57 22.2270.21 comma-styleno-unused-vars 6,3957,765 46.4851.87 52.1156.  
valid-typeof 539 51.85 51.85 no-undef 10,636 22.65 27\.  
no-self-assign 610 34.43 44.26 no-invalid-this 16,166 37.48 44\.  
Sum/W. Avg. 104,561 49.58 54\.  
...
major error type “no-invalid-this” has 16,166 instances while the  
least error type “no-new-symbol” has only 10 instances. As such, it  
...
model size (∼ 3\. 5 ×of CodeT5-base: 770M vs. 220M). If we focus on a  
...
ment (49.35→54.30) over T5-large. Based on the weighted average  
...
CodeT5 models with code-aware pretraining on large-scale source
...
\# Ground-truth EL Removal 2,381 2,381 2,  
\# Predicted EL Removal 1,882 1,925 1,  
\# Correct EL Removal 1,811 1,858 1,  
\# False Positive 71 67 56  
Precision (%) 96.23 96.52 97\.  
Recall (%) 76.06 78.03 78\.  
F1 (%) 84.96 86.30 86\.  
\`\`\`  
\`\`\`  
...
(49.58→54.15 EM). This indicates retrieval-augmented generation  
...
6,793 instances^6 in Fig. 4\. We observe that RAP-Gen significantly  
improves error removal accuracy over T5-large (69.30→78.80). The  
...
\`\`\`  
(^6) Some source files are unavailable to reproduce this metric on the full test set.  
Table 6: Performance of RAP-Gen on the Code Refinement.  
Model Small Medium  
EM BLEU-4 EM BLEU-  
Naive Copy 0.00 78.06 0.00 90\.  
LSTM 10.00 76.76 2.50 72\.  
Transformer 14.70 77.21 3.70 89\.  
RoBERTa (code) 15.90 77.30 4.10 90\.  
CodeBERT 16.40 77.42 5.16 91\.  
GraphCodeBERT 17.30 80.02 9.10 91\.  
PLBART 19.21 77.02 8.98 88\.  
CoTexT 21.58 77.28 13.11 88\.  
NSEdit 24.04 71.06 13.87 85\.  
CodeT5 21.61 77.43 13.96 87\.  
RAP-Gen 24.80 78.28 15.84 90\.
...
Table 4\. RAP-Gen outperforms the previous SoTA T5-large in 40/  
error types. Especially for the major error type “no-invalid-this”,  
RAP-Gen improves its exact match from T5-large’s 37.48 to 44.13,  
...
a lowest false positive count of 56 compared to CodeT5’s 67 and  
T5-large’s 71\. This indicates that RAP-Gen is able to learn more  
...
\`\`\`  
\#\#\# 5.3 RQ3: Comparative Study with DL-based

\#\#\# APR Models on Code Refinement

\`\`\`  
We report the comparison results on Code Refinement in Table 6\.  
All baseline results are directly obtained from their original papers.  
\`\`\`
...
No Retriever 53.46 21.61 13\.  
Random 52.98 21.25 13\.  
BM25 53.88 23.82 15\.  
CodeBERT 52.96 22.28 15\.  
CodeT5 53.93 24.37 15\.  
Hybrid (BM25+CodeT5) 54.15 24.80 15\.  
\`\`\`  
We first observe that “Naive Copy” gives a pretty high BLEU-4 score  
...
one with a best result (24.04 EM) on the small subset and CodeT  
...
downgrade compared to “no retriever” implies that randomly re-  
...
Code Refinement, where the latter employs obfuscated identifiers

\`\`\`  
Table 8: Lexical (BLEU-4) and semantic (CosSim) retrieval  
matching results on TFix and Code Refinement benchmarks.  
\`\`\`  
\`\`\`  
Retriever TFix Refine-Small Refine-Medium  
BLEU-4 CosSim BLEU-4 CosSim BLEU-4 CosSim  
Random 0.1 35.5 14.6 35.4 14.6 30\.  
BM25 23.7 70.9 41.5 68.5 39.0 66\.  
DPR 21.7 75.4 54.4 84.9 44.3 81\.  
Hybrid 24.4 73.4 57.4 84.2 45.0 80\.  
\`\`\`  
\`\`\`  
...
\`\`\`  
\#\#\# 5.5 RQ5: Comparative Study with DL-based
...
competitive to the SoTA models on both v1.2 (48 vs. Recoder’s 49\)  
and v2.0 (26 vs. SelfAPR’s 28). Considering both v1.2 and v2.0 bugs,  
it repairs 74 bugs in total, surpassing either Recoder’s 68 or Self-  
APR’s 67 bugs. Overall, both results with or without perfect FL  
...
RAP-Gen repairs 13 and 12 unique bugs for Defects4J v1.2 and v2.  
...
to other top-performing APR approaches. We further show a case  
\`\`\`
...
\`\`\`  
\`\`\`  
in Fig. 3 (c) and find that RAP-Gen successfully fixes the Chart-  
bug but in a different form with the developer’s fix.  
\`\`\`  
...
sample from the codebase for each rule or fix pattern (denoted as P  
to P16) and examine whether it can improve CodeT5’s performance  
...
in RAP-Gen are generally helpful in correcting CodeT5’s predictions  
...
fix patterns, we find that the P13 of “insert an existing block” and  
P14 of “delete statement” are applicable to most bugs, indicating  
...
in software engineering to reduce developers’ manual efforts in  
...
s/a4e95baee01bba14bf4b) to reproduce the results in this paper.  
\`\`\`
...
Unified Pre-training for Program Understanding and Generation. InNAACL-HLT.  
Association for Computational Linguistics, 2655–2668.  
\[2\]Miltiadis Allamanis, Henry Jackson-Flux, and Marc Brockschmidt. 2021\. Self-  
Supervised Bug Detection and Repair. InNeurIPS. 27865–27876.  
\[3\]Earl T. Barr, Yuriy Brun, Premkumar T. Devanbu, Mark Harman, and Federica  
Sarro. 2014\. The plastic surgery hypothesis. InSIGSOFT FSE. ACM, 306–317.  
\[4\]Berkay Berabi, Jingxuan He, Veselin Raychev, and Martin T. Vechev. 2021\. TFix:  
Learning to Fix Coding Errors with a Text-to-Text Transformer. InICML (Pro-  
ceedings of Machine Learning Research, Vol. 139). PMLR, 780–791.  
\[5\]Nghi Bui, Yue Wang, and Steven C. H. Hoi. 2022\. Detect-Localize-Repair: A  
Unified Framework for Learning to Debug with CodeT5. InEMNLP (Findings).  
Association for Computational Linguistics, 812–823.  
...
\`\`\`  
2021\. Evaluating Large Language Models Trained on Code.CoRRabs/2107.  
(2021).  
...
Pre-Trained Model for Programming and Natural Languages. InEMNLP (Findings)  
...
2017\. Convolutional Sequence to Sequence Learning. InICML (Proceedings of  
Machine Learning Research, Vol. 70). PMLR, 1243–1252.  
...
Guided Neural Machine Translation. InAAAI. AAAI Press, 5133–5140.  
...
Data Flow. InICLR. OpenReview.net.  
\[15\]Tatsunori B. Hashimoto, Kelvin Guu, Yonatan Oren, and Percy Liang. 2018\. A  
Retrieve-and-Edit Framework for Predicting Structured Outputs. InNeurIPS.  
10073–10083.  
...
former through a Neural-Symbolic Edit Grammar.CoRRabs/2204.06643 (2022).  
\[18\]Gautier Izacard and Edouard Grave. 2021\. Leveraging Passage Retrieval with  
Generative Models for Open Domain Question Answering. InEACL. Association  
for Computational Linguistics, 874–880.  
\[19\]Jiajun Jiang, Yingfei Xiong, Hongyu Zhang, Qing Gao, and Xiangqun Chen. 2018\.  
Shaping program repair space with existing patches and similar code. InISSTA.  
ACM, 298–309.  
\[20\]Nan Jiang, Thibaud Lutellier, and Lin Tan. 2021\. CURE: Code-Aware Neural  
Machine Translation for Automatic Program Repair. InICSE. IEEE, 1161–1173.  
\[21\]Harshit Joshi, José Pablo Cambronero Sánchez, Sumit Gulwani, Vu Le, Ivan  
Radicek, and Gust Verbruggen. 2022\. Repair Is Nearly Generation: Multilingual  
Program Repair with LLMs.CoRRabs/2208.11640 (2022).  
\[22\]René Just, Darioush Jalali, and Michael D. Ernst. 2014\. Defects4J: a database of  
existing faults to enable controlled testing studies for Java programs. InISSTA.  
ACM, 437–440.
...
Open-Domain Question Answering. InEMNLP (1). Association for Computational  
Linguistics, 6769–6781.  
\[24\]Dongsun Kim, Jaechang Nam, Jaewoo Song, and Sunghun Kim. 2013\. Automatic  
patch generation learned from human-written patches. InICSE. IEEE Computer  
Society, 802–811.  
...
and Deep Reinforcement Learning. InNeurIPS.  
\[26\]Xuan-Bach Dinh Le, David Lo, and Claire Le Goues. 2016\. History Driven Program  
Repair. InSANER. IEEE Computer Society, 213–224.  
...
tion for Knowledge-Intensive NLP Tasks. InNeurIPS.  
\[28\]Huayang Li, Yixuan Su, Deng Cai, Yan Wang, and Lemao Liu. 2022\. A Survey on  
Retrieval-Augmented Text Generation.CoRRabs/2202.01110 (2022).  
\[29\]Jia Li, Yongmin Li, Ge Li, Xing Hu, Xin Xia, and Zhi Jin. 2021\. EditSum: A  
Retrieve-and-Edit Framework for Source Code Summarization. InASE. IEEE,  
155–166.  
\[30\]Jian Li, Yue Wang, Michael R. Lyu, and Irwin King. 2018\. Code Completion with  
Neural Attention and Pointer Networks. InIJCAI. ijcai.org, 4159–4165.  
\[31\]Yi Li, Shaohua Wang, and Tien N. Nguyen. 2020\. DLFix: context-based code  
transformation learning for automated program repair. InICSE. ACM, 602–614.  
\[32\]Yi Li, Shaohua Wang, and Tien N. Nguyen. 2022\. DEAR: A Novel Deep Learning-  
based Approach for Automated Program Repair. InICSE. ACM, 511–523.  
...
using ensemble for program repair. InISSTA. ACM, 101–114.  
\[41\]Diego Marcilio, Carlo A. Furia, Rodrigo Bonifácio, and Gustavo Pinto. 2020\.  
...
\[42\]Monperrus Martin, Simon Urli, Thomas Durieux, Matias Martinez, Benoît Baudry,  
...
\[44\]Kivanç Muslu, Yuriy Brun, Reid Holmes, Michael D. Ernst, and David Notkin. 2012\.  
Speculative analysis of integrated development environment recommendations.  
InOOPSLA. ACM, 669–682.  
\[45\]Wonseok Oh and Hakjoo Oh. 2022\. PyTER: effective program repair for Python  
type errors. InESEC/SIGSOFT FSE. ACM, 922–934.  
...
InEMNLP (Findings). Association for Computational Linguistics, 2719–2734.  
...
\`\`\`  
2009\. Automatically patching errors in deployed software. InSOSP ’09.  
...
\[50\]Julian Aron Prenner, Hlib Babii, and Romain Robbes. 2022\. Can OpenAI’s Codex  
Fix Bugs?: An evaluation on QuixBugs. InAPR@ICSE. IEEE, 69–75.  
\[51\]Yuhua Qi, Xiaoguang Mao, Yan Lei, Ziying Dai, and Chengsong Wang. 2014\. The  
strength of random search on automated program repair. InICSE. ACM, 254–265.  
...
\[55\]André Riboira and Rui Abreu. 2010\. The GZoltar Project: A Graphical Debugger  
Interface. InTAIC PART (Lecture Notes in Computer Science, Vol. 6303). Springer,  
215–218.  
...
Translation of Rare Words with Subword Units. InACL (1). The Association for  
...
works. InACL (1). The Association for Computer Linguistics, 1556–1566.  
\[60\]Shin Hwei Tan and Abhik Roychoudhury. 2015\. relifix: Automated repair of  
software regressions. In2015 IEEE/ACM 37th IEEE International Conference on  
Software Engineering, Vol. 1\. IEEE, 471–482.  
\[61\]Shin Hwei Tan, Hiroaki Yoshida, Mukul R. Prasad, and Abhik Roychoudhury. 2016\.  
Anti-patterns in search-based program repair. InSIGSOFT FSE. ACM, 727–738.  
\[62\]Kristín Fjóla Tómasdóttir, Mauricio Finavaro Aniche, and Arie van Deursen. 2017\.  
Why and how JavaScript developers use linters. InASE. IEEE Computer Society,  
578–589.  
\[63\]Nikzad Benny Toomarian and Jacob Barhen. 1992\. Learning a trajectory using  
adjoint functions and teacher forcing.Neural Networks5, 3 (1992), 473–484.  
...
neering in Practice Track (ICSE-SEIP)(2017), 95–104.  
\[66\]Aäron van den Oord, Yazhe Li, and Oriol Vinyals. 2018\. Representation Learning  
with Contrastive Predictive Coding.CoRRabs/1807.03748 (2018).  
\[67\]Rijnard van Tonder and Claire Le Goues. 2018\. Static automated program repair  
for heap properties. InICSE. ACM, 151–162.  
...
standing and Generation. InEMNLP (1). Association for Computational Linguis-  
tics, 8696–8708.  
...
International Conference on Software Engineering(2009), 364–374.  
\[72\]Cathrin Weiß, Rahul Premraj, Thomas Zimmermann, and Andreas Zeller. 2007\.  
How Long Will It Take to Fix This Bug?. InMSR. IEEE Computer Society, 1\.  
\[73\]Ming Wen, Junjie Chen, Rongxin Wu, Dan Hao, and Shing-Chi Cheung. 2018\.  
Context-aware patch generation for better automated program repair. InICSE.  
ACM, 1–11.  
...
Software Analysis, Evolution and Reengineering (SANER)(2019), 479–490.  
...
with Execution-based Backpropagation. InICSE. ACM, 1506–1518.  
\[77\]Hiroaki Yoshida, Rohan Bavishi, Keisuke Hotta, Yusuke Nemoto, Mukul R. Prasad,  
and Shinji Kikuchi. 2020\. Phoenix: a tool for automated data-driven synthesis of  
repairs for static analysis violations. InICSE (Companion Volume). ACM, 53–56.  
...
ESEC/SIGSOFT FSE. ACM, 341–353.

\`\`\`  
Received 2023-02-02; accepted 2023-07-  
\`\`\`
