...
ally accessible tokens —bridge tokensandmem-  
ory tokens— to improve performance and ef-  
ficiency.Bridge tokensare inserted throughout  
the input sequence to aggregate local information  
and facilitate global interaction, whilememory  
tokensare included to highlight important state-  
...
\`\`\`  
\*Equal contribution (^1) Sun Yat-sen University (^2) University of  
California, San Diego^3 Microsoft Research Asia. Correspondence  
to: Daya Guo\<guody5@mail2.sysu.edu.cn\>.  
Proceedings of the 40 thInternational Conference on Machine  
Learning, Honolulu, Hawaii, USA. PMLR 202, 2023\. Copyright  
2023 by the author(s).  
An efficient model that can scale to such long input can be  
...
file, we introducebridge attentionandglobal attention, with  
the corresponding two types of globally accessible tokens,  
bridge tokensandmemory tokens. Bridge attention aggre-  
...
future research onLongCodeCompletion, we construct a  
...
\# arXiv:2306.14893v1 \[cs.SE\] 26 Jun 2023
...
"""  
def \_\_init\_\_(self, in\_channels, out\_channels, kernel\_size, stride, padding):  
super(ConvBlock, self).\_\_init\_\_()  
self.conv \= nn.Conv2d(in\_channels, out\_channels, kernel\_size, stride, padding)  
...
self.conv\_block1 \= ConvBlock( 3 , 32 , 3 , 1 , 1 ) \#first block  
self.conv\_block2 \= ConvBlock( 32 , 64 , 3 , 1 , 1 ) \#second block  
self.flatten \= nn.Flatten()  
self.fc1 \= nn.Linear(  
in\_features= 64 \* 8 \* 8 , out\_features= 128 )  
self.fc2 \= nn.Linear(  
in\_features= 128 , out\_features= 10 )  
\`\`\`  
\`\`\`  
\<bridge\>  
\`\`\`  
...
\`\`\`  
def forward(self, x):  
x \= self.conv\_block1(x)  
x \= self.conv\_block2(x)  
x \= self.flatten(x)  
x \= F.relu(self.fc1(x))  
x \= self.fc2(x)  
return x  
...
(^1) All the codes and data are available athttps://github.  
com/microsoft/CodeBERT.  
n-gram techniques (Tu et al., 2014; Hindle et al., 2016\) and  
...
sequences since its time and space complexity isO(n^2 ),  
wherenis the length of the sequence. Prior studies focus  
...
completion datasets fromPY150(Raychev et al., 2016\) in  
Python andGithub Java CorpusAllamanis & Sutton (2013)  
...
the input length.  
In this paper, we introduce theLongCodeCompletion  
...
our datasets from thegithub-code^2 dataset, which contains  
...
\- We first follow Allamanis (2019) to deduplicate exam-  
    ...
    using a standard compiler tool calledtree-sitter.^3  
...
(^3) https://github.com/tree-sitter/  
tree-sitter
...
rent statement, whereas to generate variables such asxand  
convblock, the model needs to look at neighboring state-  
...
\`\`\`  
\`\`\`  
Figure 2.Distribution of average attention scores between two  
...
\`\`\`  
\`\`\`  
Formally, given the linear projectionsQ,K,V, the self-  
...
\`\`\`  
whereMis a mask matrix (to be completed in Equation 5\)  
to control the context a token can attend to when computing  
its contextual representation. If thei-th token is allowed to  
attend to thej-th token, thenMijis set to 0, otherwise−∞.  
For window attention, the mask attention matrixMwindow  
...
\`\`\`  
wherewis the window size. This window attention pattern  
reduces the complexity of the self-attention mechanism by  
limiting the receptive field size of each token to a small  
window of sizewat each layer. The computation complexity  
of this pattern isO(n×w), which scales linearly with input  
sequence lengthn. After applyingNtransformer layers  
of such sliding window attention, the receptive field size  
increases toN×wat the top layer. Since each token only  
attends towtokens to its left rather than the entire preceding  
...
ofLtokens away, it would require⌈Lw⌉hops through win-  
...
token, namelybridge tokens, to aggregate local information  
...
Specifically, we insertmbridge tokensSbevery⌈mn⌉tokens  
...
aggregate information from the preceding⌈mn⌉tokens. The  
...
The complexity of bridge attention isO(m×n)≈O(n)  
wherem≪n. Compared to stacked window attention,  
...
portedtorch.nn.functionalas a new identifierF,  
the model cannot be sure whether to useFor the full

\`\`\`  
package name.^4 Also, to call the forward function of  
convblock1andconvblock2, the model needs ac-  
cess to the original definition of the classConvBlock,  
...
we add another type of global token, namelymemory tokens,  
to inject code heuristics to the attention. Specifically, we  
leverage the structure of code withtree-sitterto parse the  
...
line feeds asG, wherek=|G|. The mask matrixMglobal  
...
\`\`\`  
The complexity of the global attention isO(kn)≈O(n),  
as we havek≪n, wherenis the length of the sequence.  
...
\`\`\`  
\`\`\`  
wheremaxis the element-wise maximum function.  
\`\`\`  
\#\# 5\. Experiments
...
\`\`\`  
(^4) Both are common code styles in PyTorch.

\`\`\`  
Table 2.Experimental results on the Long Code Completion (LCC) dataset.  
\`\`\`  
\`\`\`  
Model \#Param. Memory Runtime  
Python Java C\#  
EM Edit Sim EM Edit Sim EM Edit Sim  
OpenAI Codex 12B \- \- 39.65 68.97 43.15 72.05 53.89 77\.  
Transformer 124M 191M 750ms 10.64 43.64 15.32 47.52 19.16 48\.  
GPT-2 124M 191M 750ms 11.20 42.62 17.09 47.18 20.27 48\.  
CodeGPT 124M 191M 750ms 12.24 43.81 19.20 49.50 22.58 51\.  
UniXcoder 126M 191M 750ms 16.55 50.22 23.93 55.38 27.97 57\.  
LongFormer 150M 381M 781ms 16.79 51.07 24.80 56.03 29.75 58\.  
BigBird 128M 205M 804ms 17.03 51.14 25.19 56.91 30.27 58\.  
LongCoder 150M 211M 812ms 17.88 55.07 26.42 61.21 31.34 64\.  
\`\`\`  
\- w/o pretrain 150M 211M 812ms 17.61 54.82 25.96 60.81 31.22 64\.
...
performance of LongCoder on the cross-file-random (XF-
...
LongCoder 43.77 73.37 33.13 67\.  
\`\`\`  
\`\`\`  
...
Model Python Java  
EM Edit Sim EM Edit Sim  
Transformer 7.0 38.3 5.8 34\.  
GPT-2 15.5 48.2 11.6 41\.  
CodeGPT 16.6 49.1 13.3 44\.  
UniXcoder 18.0 53.4 16.5 50\.  
LongCoder 21.4 59.7 19.7 60\.  
\`\`\`  
\`\`\`  
...
Python Java C\#  
EM Edit Sim EM Edit Sim EM Edit Sim  
LongCoder 17.61 54.82 25.96 60.81 31.22 64\.  
w/o memory tokens 16.80 53.90 24.87 60.35 30.16 63\.  
w/o bridge tokens 17.54 51.92 25.88 57.48 30.80 59\.  
w/o out-of-window context 16.66 50.63 24.17 55.97 28.81 57\.  
w/ equidistant memory tokens 17.16 54.25 24.96 60.42 30.31 63\.  
\`\`\`  
...
non-sparse models, we set the window sizewto 512 so that  
...
tokensnand global tokenskas 16 and 64, respectively.  
...
in Exact Match score and 4.0%–6.0% in Edit Similarity,

\`\`\`  
which reveal the effectiveness of our proposed bridge and  
...
\`\`\`  
\`\`\`  
Out-of-Window Context In-Window Context  
\`\`\`  
\`\`\`  
timestamp \= timestamp\_now()  
return (status\_headers, stream)  
timestamp \= time.time()  
...
\`\`\`  
... (22 lines omitted)  
def fetch\_request(self, url, urlrewriter,  
head\_insert\_func=None,  
urlkey=None,  
env=None,  
req\_headers={},  
timestamp=None,  
follow\_redirects=False,  
ignore\_proxies=False,  
verify=True,  
remote\_only=True):  
... (28 lines omitted)  
...
import requests  
... (8 lines omitted)  
from pywb.utils.timeutils import timestamp\_now  
... (5 lines omitted)  
class LiveRewriter(object):  
... (94 lines omitted)  
def fetch\_http(self, url,  
urlkey=None,  
env=None,  
req\_headers=None,  
follow\_redirects=False,  
...
\`\`\`  
\`\`\`  
public StringBuffer String fieldsToXMLStringsb \= new StringBuffer(java.();util.HashMap domMap){  
if sb(this.append.getClinicalContact("\<clinicalContact\>"() \!= );null){  
sb.append(this.getClinicalContact().toXMLString(domMap));  
sb } .append("\</clinicalContact\>");  
if sb(this.append.getAuthoringDateTime("\<authoringDateTime\>"() \!= );null){  
sb this.append.getAuthoringDateTime(new ims.framework.()).utilstoString.DateTime((  
ims sb..appendframework("\</authoringDateTime\>".utils.DateTimeFormat);.MILLI));  
}  
if sb(this.append.getAuthoringCP("\<authoringCP\>"() \!= );null){}  
sb sb..appendappend((this"\</authoringCP\>".getAuthoringCP);().toXMLString(domMap));  
} if (this.getGaitAspect() \!= null){  
if sb(this.append.getGaitAspect("\<gaitAspect\>"().size);() \> 0 ){  
sb.append(ims.domain.lookups.LookupInstance.toXMLString(  
this sb.append.getGaitAspect("\</gaitAspect\>"())););  
}  
}  
\`\`\`  
\`\`\`  
package public class ims.therapiesGaitReEducation .treatmentextends .domain... .objectsimplements ; ... {  
public static final ... (20 lines omitted)int CLASSID \= 1044100009 ;  
(Methods omitted)  
public public GaitReEducation GaitReEducation ((){...}Integer id, int ver){...}  
public public GaitReEducation Class getRealDomainClass(...){...}(){...}  
public public ...void ClinicalContact getClinicalContactsetClinicalContact(...ClinicalContact clinicalContact() {...} ) {...}  
public public javavoid .utilsetAuthoringDateTime.Date getAuthoringDateTime(java.util() {...}.Date authoringDateTime) {...}  
public ims.core.resource.people.domain.objects.Hcp getAuthoringCP() {...}  
public public void java.setAuthoringCPutil.List getGaitAspect(...Hcp authoringCP() {...}) {...}  
public public void String setGaitAspectgetDetails() {(java.util.List paramValue) {...}  
return } details;  
public public void ims.domainsetDetails.SystemInformation getSystemInformation(String details) {...} () {...}  
public static boolean isConfigurationObject(){...}  
public public int String getClassIdgetClassVersion() {...}(){...}  
public public String String toAuditStringtoXMLString(){...}(){...}  
...
to thetimestampvariable. However, only LongCoder  
...
thetimestampnowfunction. Codex-2048 uses a long  
...
aims to convert aHashMapvariable into an XML string.  
The function sequentially calls the getter functions of the  
GaitReEducationclass and has already completed call-  
ing thegetGaitAspectfunction. From the out-window  
context, it is clear that the next call should be made to the  
getDetailsfunction. In order to correctly complete the

\`\`\`  
function, it is essential to keep track of all function defini-  
...
tion can be “self-fulfilling”. For example, GitHub CoPilot^5  
...
machine learning models of code. InOnward\!, pp. 143–
...
code. InSIGSOFT FSE, pp. 472–483. ACM, 2014\.
...
Bielik, P., Raychev, V., and Vechev, M. T. PHOG: proba-  
bilistic model for code. InICML, volume 48 ofJMLR  
...
models are few-shot learners. InNeurIPS, 2020\.  
...
X., Gane, A., Sarlos, T., Hawkins, P., Davis, J. Q., Mo- ́  
...
window access by syntax hierarchy. InEMNLP, pp. 4713–
...
guage understanding. InNAACL-HLT, pp. 4171–4186.  
...
and generation. InNeurIPS, pp. 13042–13054, 2019\.  
\`\`\`  
\`\`\`  
...
representation. InACL, pp. 7212–7225. Association for  
...
ers with linear attention. InICML, volume 119 ofPro-  
ceedings of Machine Learning Research, pp. 5156–5165.  
PMLR, 2020\.

Kitaev, N., Kaiser, L., and Levskaya, A. Reformer: The  
efficient transformer. InICLR. OpenReview.net, 2020\.
...
in attention. InICLR. OpenReview.net, 2022\.
...
model for code with decision trees. InOOPSLA, pp.  
731–747. ACM, 2016\.

Svyatkovskiy, A., Deng, S. K., Fu, S., and Sundaresan, N.  
Intellicode compose: code generation using transformer.  
InESEC/SIGSOFT FSE, pp. 1433–1443. ACM, 2020\.
...
software. InSIGSOFT FSE, pp. 269–280. ACM, 2014\.  
\`\`\`  
\`\`\`  
Vaswani, A., Shazeer, N., Parmar, N., Uszkoreit, J., Jones,  
L., Gomez, A. N., Kaiser, L., and Polosukhin, I. Attention  
is all you need. InNIPS, pp. 5998–6008, 2017\.  
\`\`\`  
\`\`\`  
...
berti, C., Ontan ̃on, S., Pham, P., Ravula, A., Wang, Q., ́  
Yang, L., and Ahmed, A. Big bird: Transformers for  
longer sequences. InNeurIPS, 2020\.  
\`\`\`
